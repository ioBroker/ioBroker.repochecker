...
async function checkTagTriggeredRun(context, workflowRuns, version, taggedVersion, codeBase, sourceLabel) {
    ...
    if (tagRun.conclusion === 'cancelled') {
        ...
        try {
            ...
            if (!checkAndLintJob) {
                context.warnings.push(
                    `[W3050] "${WORKFLOW_FILE_SHORT}": no "check-and-lint" job found in ${formatRunLink(tagRun)}. ` +
                ...
                if (!checkAndLintLog) {
                    context.warnings.push(
                        `[W3050] "${WORKFLOW_FILE_SHORT}": could not retrieve log for "check-and-lint" job in ${formatRunLink(tagRun)}.`,
                    );
                } else {
                    ...
                    if (missingCheckLines.length > 0) {
                        context.warnings.push(
                            `[S3051] "${WORKFLOW_FILE_SHORT}": output of "testing-action-check" in ${formatRunLink(tagRun)} ` +
            ...
            if (!context.cfg.onlyWWW) {
                ...
                if (adapterTestJobs.length === 0) {
                    context.warnings.push(
                        `[W3052] "${WORKFLOW_FILE_SHORT}": no "adapter-tests" job found in ${formatRunLink(tagRun)}. ` +
                    ...
                    for (const adapterJob of adapterTestJobs) {
                        ...
                        if (!adapterLog) {
                            context.warnings.push(
                                `[W3052] "${WORKFLOW_FILE_SHORT}": could not retrieve log for job "${adapterJob.name}" in ${formatRunLink(tagRun)}.`,
                            );
                        } else {
                            ...
                            if (missingAdapterLines.length > 0) {
                                context.warnings.push(
                                    `[S3053] "${WORKFLOW_FILE_SHORT}": output of "testing-action-adapter" job "${adapterJob.name}" in ${formatRunLink(tagRun)} ` +
        ...
        if (!Array.isArray(workflowRuns) || workflowRuns.length === 0) {
            ...
            if (!Array.isArray(defaultBranchRuns) || defaultBranchRuns.length === 0) {
                ...
                try {
                    ...
                    if (!checkAndLintJob) {
                        context.warnings.push(
                            `[W3050] "${WORKFLOW_FILE_SHORT}": no "check-and-lint" job found in ${formatRunLink(lastRun)}. ` +
                                `Cannot verify "npm run test:package" output.`,
                        );
                    } else {
                        const checkAndLintLog = await fetchJobLog(context, checkAndLintJob.id);
                        if (!checkAndLintLog) {
                            context.warnings.push(
                                `[W3050] "${WORKFLOW_FILE_SHORT}": could not retrieve log for "check-and-lint" job in ${formatRunLink(lastRun)}.`,
                            );
                        } else {
                            ...
                            if (missingCheckLines.length > 0) {
                                context.warnings.push(
                                    `[S3051] "${WORKFLOW_FILE_SHORT}": output of "testing-action-check" in ${formatRunLink(lastRun)} ` +
                                        `does not match the expected defaults for "npm run test:package". ` +
                                        `Please check the log manually: [${checkAndLintJob.html_url}](${checkAndLintJob.html_url})`,
                                );
                                if (common.isDebug()) {
                    ...
                    if (!context.cfg.onlyWWW) {
                        ...
                        if (adapterTestJobs.length === 0) {
                            context.warnings.push(
                                `[W3052] "${WORKFLOW_FILE_SHORT}": no "adapter-tests" job found in ${formatRunLink(lastRun)}. ` +
                                    `Cannot verify "npm run test:integration" output.`,
                            );
                        } else {
                            for (const adapterJob of adapterTestJobs) {
                                ...
                                if (!adapterLog) {
                                    context.warnings.push(
                                        `[W3052] "${WORKFLOW_FILE_SHORT}": could not retrieve log for job "${adapterJob.name}" in ${formatRunLink(lastRun)}.`,
                                    );
                                } else {
                                    const missingAdapterLines = findMissingExpectedLines(
                                        EXPECTED_TEST_INTEGRATION_LINES,
                                    ...
                                    if (missingAdapterLines.length > 0) {
                                        context.warnings.push(
                                            `[S3053] "${WORKFLOW_FILE_SHORT}": output of "testing-action-adapter" job "${adapterJob.name}" in ${formatRunLink(lastRun)} ` +
                                                `does not match the expected defaults for "npm run test:integration". ` +
                                                `Please check the log manually: [${adapterJob.html_url}](${adapterJob.html_url})`,
                                        );
                                        if (common.isDebug()) {
        ...
        if (context.cfg.onlyWWW) {
            ...
            if (missingSystems.length === 0) {
                ...
                if (uncoveredMissingSystems.length > 0) {
                    context.warnings.push(
                        `[W3027] "${WORKFLOW_FILE_SHORT}": testing for OS ${uncoveredMissingSystems.join(', ')} is missing in job "adapter-tests". ` +
