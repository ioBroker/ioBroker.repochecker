...
const EXPECTED_TEST_PACKAGE_LINES = [
    ...
    '✔ property main is defined for non onlyWWW adapters',
    ...
    '✔ The adapter uses a supported admin UI',
...
async function checkTagTriggeredRun(context, workflowRuns, version, taggedVersion, codeBase, sourceLabel) {
    ...
    if (tagRun.conclusion === 'cancelled') {
        ...
        try {
            ...
            if (!checkAndLintJob) {
                context.warnings.push(
                    `[W3050] Workflow "${WORKFLOW_FILE_SHORT}": no "check-and-lint" job found in ${formatRunLink(tagRun)}. ` +
                ...
                if (!checkAndLintLog) {
                    context.warnings.push(
                        `[W3050] Workflow "${WORKFLOW_FILE_SHORT}": could not retrieve log for "check-and-lint" job in ${formatRunLink(tagRun)}.`,
                    );
                } else {
                    ...
                    if (missingCheckLines.length > 0) {
                        context.warnings.push(
                            `[S3051] Workflow "${WORKFLOW_FILE_SHORT}": output of "testing-action-check" in ${formatRunLink(tagRun)} ` +
                                `does not match the expected defaults for "npm run test:package". ` +
                                `Please check the log manually: [${checkAndLintJob.html_url}](${checkAndLintJob.html_url})`,
                        );
                        if (common.isDebug()) {
                            common.debug(
                                `[S3051] Missing lines in check-and-lint log (${missingCheckLines.length}):\n${missingCheckLines.map(l => `  - ${l}`).join('\n')}`,
                            );
                        }
                    } else {
                        context.checks.push(
            ...
            if (!context.cfg.onlyWWW) {
                ...
                if (adapterTestJobs.length === 0) {
                    context.warnings.push(
                        `[W3052] Workflow "${WORKFLOW_FILE_SHORT}": no "adapter-tests" job found in ${formatRunLink(tagRun)}. ` +
                    ...
                    for (const adapterJob of adapterTestJobs) {
                        ...
                        if (!adapterLog) {
                            context.warnings.push(
                                `[W3052] Workflow "${WORKFLOW_FILE_SHORT}": could not retrieve log for job "${adapterJob.name}" in ${formatRunLink(tagRun)}.`,
                            );
                        } else {
                            ...
                            if (missingAdapterLines.length > 0) {
                                context.warnings.push(
                                    `[S3053] Workflow "${WORKFLOW_FILE_SHORT}": output of "testing-action-adapter" job "${adapterJob.name}" in ${formatRunLink(tagRun)} ` +
                                        `does not match the expected defaults for "npm run test:integration". ` +
                                        `Please check the log manually: [${adapterJob.html_url}](${adapterJob.html_url})`,
                                );
                                if (common.isDebug()) {
                                    common.debug(
                                        `[S3053] Missing lines in adapter-tests job "${adapterJob.name}" log (${missingAdapterLines.length}):\n${missingAdapterLines.map(l => `  - ${l}`).join('\n')}`,
                                    );
                                }
                            } else {
                                context.checks.push(
        ...
        if (!Array.isArray(workflowRuns) || workflowRuns.length === 0) {
            ...
            if (!Array.isArray(defaultBranchRuns) || defaultBranchRuns.length === 0) {
                ...
                try {
                    ...
                    if (!checkAndLintJob) {
                        context.warnings.push(
                            `[W3050] Workflow "${WORKFLOW_FILE_SHORT}": no "check-and-lint" job found in ${formatRunLink(lastRun)}. ` +
                                `Cannot verify "npm run test:package" output.`,
                        );
                    } else {
                        const checkAndLintLog = await fetchJobLog(context, checkAndLintJob.id);
                        if (!checkAndLintLog) {
                            context.warnings.push(
                                `[W3050] Workflow "${WORKFLOW_FILE_SHORT}": could not retrieve log for "check-and-lint" job in ${formatRunLink(lastRun)}.`,
                            );
                        } else {
                            ...
                            if (missingCheckLines.length > 0) {
                                context.warnings.push(
                                    `[S3051] Workflow "${WORKFLOW_FILE_SHORT}": output of "testing-action-check" in ${formatRunLink(lastRun)} ` +
                                        `does not match the expected defaults for "npm run test:package". ` +
                                        `Please check the log manually: [${checkAndLintJob.html_url}](${checkAndLintJob.html_url})`,
                                );
                                if (common.isDebug()) {
                                ...
                                context.checks.push(
                                    `Job "check-and-lint" log contains expected "npm run test:package" output in ${formatRunLink(lastRun)}.`,
                                );
                            }
                        }
                    }
                    ...
                    if (!context.cfg.onlyWWW) {
                        ...
                        if (adapterTestJobs.length === 0) {
                            context.warnings.push(
                                `[W3052] Workflow "${WORKFLOW_FILE_SHORT}": no "adapter-tests" job found in ${formatRunLink(lastRun)}. ` +
                                    `Cannot verify "npm run test:integration" output.`,
                            );
                        } else {
                            for (const adapterJob of adapterTestJobs) {
                                ...
                                if (!adapterLog) {
                                    context.warnings.push(
                                        `[W3052] Workflow "${WORKFLOW_FILE_SHORT}": could not retrieve log for job "${adapterJob.name}" in ${formatRunLink(lastRun)}.`,
                                    );
                                } else {
                                    const missingAdapterLines = findMissingExpectedLines(
                                        EXPECTED_TEST_INTEGRATION_LINES,
                                    ...
                                    if (missingAdapterLines.length > 0) {
                                        context.warnings.push(
                                            `[S3053] Workflow "${WORKFLOW_FILE_SHORT}": output of "testing-action-adapter" job "${adapterJob.name}" in ${formatRunLink(lastRun)} ` +
                                                `does not match the expected defaults for "npm run test:integration". ` +
                                                `Please check the log manually: [${adapterJob.html_url}](${adapterJob.html_url})`,
                                        );
                                        if (common.isDebug()) {
                                        ...
                                        context.checks.push(
                                            `Job "${adapterJob.name}" log contains expected "npm run test:integration" output in ${formatRunLink(lastRun)}.`,
                                        );
                                    }
                                }
                            }
    ...
    if (!hasLegacyTesting) {
        ...
        for (const [jobName, job] of Object.entries(jobs)) {
            ...
            for (let i = 0; i < job.steps.length; i++) {
                ...
                const message =
                    `Workflow "${WORKFLOW_FILE_SHORT}": job "${jobName}" step "${stepLabel}" uses "continue-on-error: true". ` +
        ...
        if (!jobs['check-and-lint']) {
            ...
            if (!jobHasStepWithAction(jobs['check-and-lint'], checkActionPrefix)) {
                ...
                for (const param of forbiddenCheckParams) {
                    ...
                    if (stepWithParam) {
                        context.warnings.push(
                            `[W3048] Workflow "${WORKFLOW_FILE_SHORT}": job "check-and-lint" step using "ioBroker/testing-action-check" ` +
        ...
        if (context.cfg.onlyWWW) {
            ...
            if (!jobHasStepWithAction(jobs['adapter-tests'], adapterActionPrefix)) {
                ...
                if (adapterStepWithParam) {
                    context.warnings.push(
                        `[W3049] Workflow "${WORKFLOW_FILE_SHORT}": job "adapter-tests" step using "ioBroker/testing-action-adapter" ` +
...
// [W3046] Workflow "${WORKFLOW_FILE_SHORT}": job "${jobName}" step "${stepLabel}" uses "continue-on-error: true". Workflow test-and-release should abort if any error occurs; remove "continue-on-error".
// [E3047] Workflow "${WORKFLOW_FILE_SHORT}": job "${jobName}" step "${stepLabel}" uses "continue-on-error: true". Workflow test-and-release should abort if any error occurs; remove "continue-on-error".
// [W3048] Workflow "${WORKFLOW_FILE_SHORT}": job "check-and-lint" step using "ioBroker/testing-action-check" specifies a command parameter that should not be changed.
// [W3049] Workflow "${WORKFLOW_FILE_SHORT}": job "adapter-tests" step using "ioBroker/testing-action-adapter" specifies "integration-test-command" that should not be changed.
// [W3050] Workflow "${WORKFLOW_FILE_SHORT}": no "check-and-lint" job / log found in the latest run. Cannot verify "npm run test:package" output.
// [S3051] Workflow "${WORKFLOW_FILE_SHORT}": output of "testing-action-check" does not match expected defaults. Please check the log manually.
// [W3052] Workflow "${WORKFLOW_FILE_SHORT}": no "adapter-tests" job / log found in the latest run. Cannot verify "npm run test:integration" output.
// [S3053] Workflow "${WORKFLOW_FILE_SHORT}": output of "testing-action-adapter" does not match expected defaults. Please check the log manually.
