    ...
    if (!hasLegacyTesting) {
        ...
        for (const [jobName, job] of Object.entries(jobs)) {
            ...
            for (let i = 0; i < job.steps.length; i++) {
                ...
                if (!isTruthyBoolean(step?.['continue-on-error'])) {
                    continue;
                }
...
// [W3046] Workflow "${WORKFLOW_FILE_SHORT}": job "${jobName}" step "${stepLabel}" uses "continue-on-error: true". Workflow should abort on errors.
// [E3047] Workflow "${WORKFLOW_FILE_SHORT}": job "${jobName}" step "${stepLabel}" uses "continue-on-error: true" on ioBroker/testing-action-check/adapter/deploy.
