...
async function checkTests(context) {
    ...
    if (!hasLegacyTesting) {
        ...
        if (enginesNode) {
            ...
            if (enginesMatch) {
                const enginesMajor = parseInt(enginesMatch[1], 10);
        ...
        if (context.cfg.onlyWWW) {
            ...
            if (matrixNodeVersions.length > 0) {
                const validVersionNums = testingCfg.validMatrixNodeVersions.map(v => parseInt(v, 10));
                const requiredVersionNums = testingCfg.requiredMatrixNodeVersions.map(v => parseInt(v, 10));
                const recommendedVersionNums = testingCfg.recommendedMatrixNodeVersions.map(v => parseInt(v, 10));
