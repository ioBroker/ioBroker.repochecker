...
async function checkTests(context) {
    ...
    if (!hasTesting && !hasLegacyTesting) {
        if (!context.cfg.onlyWWW) {
            ...
            // Without a testing framework, skip further workflow checks
            return context;
        }
        context.checks.push('Testing devDependency check skipped (onlyWWW adapter).');
    } else {
        context.checks.push('Testing devDependency found in package.json.');

        if (hasLegacyTesting && !hasTesting) {
    ...
    if (workflow.name !== 'Test and Release') {
        context.warnings.push(
            `[W3004] Workflow "name" in "${WORKFLOW_FILE}" should be "Test and Release" (found: "${workflow.name || '(not set)'}" ).`,
        );
    } else {
    ...
    if (!onAttr || !Object.prototype.hasOwnProperty.call(onAttr, 'push')) {
        ...
        if (!tags || !Array.isArray(tags)) {
            ...
            const missingPatterns = requiredTagPatterns.filter(pat => !tags.includes(pat));
    ...
    if (!concurrency || concurrency.group !== '${{ github.ref }}' || concurrency['cancel-in-progress'] !== true) {
        context.warnings.push(
            `[W3009] Workflow "${WORKFLOW_FILE}" is missing the recommended concurrency configuration:\n` +
    ...
    if (!hasLegacyTesting) {
        ...
        if (!jobs['check-and-lint']) {
            context.errors.push(`[E3010] Workflow "${WORKFLOW_FILE}": job "check-and-lint" is missing. Please add it.`);
        } else {
        ...
        if (!jobs['deploy']) {
            ...
            // E3016: deploy must need check-and-lint and adapter-tests (adapter-tests not required for onlyWWW)
            if (context.cfg.onlyWWW) {
                if (!jobNeedsAll(jobs['deploy'], ['check-and-lint'])) {
                    context.errors.push(
                        `[E3016] Workflow "${WORKFLOW_FILE}": job "deploy" must declare "needs" for "check-and-lint".`,
                    );
                } else {
                    context.checks.push('Job "deploy" correctly requires "check-and-lint".');
                }
            } else if (!jobNeedsAll(jobs['deploy'], ['check-and-lint', 'adapter-tests'])) {
                context.errors.push(
