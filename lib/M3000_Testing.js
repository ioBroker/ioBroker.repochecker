    ...
    if (tagRun.conclusion === 'cancelled') {
        ...
        try {
            ...
            if (isRunOlderThan90Days(tagRun)) {
                context.warnings.push(
                    `[S3056] "${WORKFLOW_FILE_SHORT}": workflow ${formatRunLink(tagRun)} for ${sourceLabel} is older than 90 days; logfiles are no longer available.`,
                );
            } else {
            ...
            if (!Array.isArray(defaultBranchRuns) || defaultBranchRuns.length === 0) {
                ...
                try {
                    ...
                    if (isRunOlderThan90Days(lastRun)) {
                        context.warnings.push(
                            `[S3056] "${WORKFLOW_FILE_SHORT}": workflow ${formatRunLink(lastRun)} on branch "${defaultBranch}" is older than 90 days; logfiles are no longer available.`,
                        );
                    } else {
...
// [S3054] "${WORKFLOW_FILE_SHORT}": log for "check-and-lint" job in ${formatRunLink(tagRun)} for ${sourceLabel} does no longer exist.`,
// [S3055] "${WORKFLOW_FILE_SHORT}": log for "${adapterJob.name}" job in ${formatRunLink(tagRun)} for ${sourceLabel} does no longer exist.`,
// [S3056] "${WORKFLOW_FILE_SHORT}": workflow ${formatRunLink(run)} for ${sourceLabel} is older than 90 days; logfiles are no longer available.
