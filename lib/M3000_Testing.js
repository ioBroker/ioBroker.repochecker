...
/**
 * Returns all action refs (part after "@") used by a job for the given action prefix.
 *
 * @param {object} job - A GitHub Actions job object
 * @param {string} actionPrefix - The action name prefix to look for (e.g. 'ioBroker/testing-action-check@')
...
function parseActionMajorVersion(ref) {
    ...
    const match = ref.trim().match(/^v?(\d+)/i);
    ...
    if (!hasLegacyTesting) {
        ...
        if (!jobs['check-and-lint']) {
            ...
            if (!jobHasStepWithAction(jobs['check-and-lint'], checkActionPrefix)) {
                ...
                if (lowerRefs.length > 0) {
                    context.warnings.push(
                        `[W3028] Workflow "${WORKFLOW_FILE_SHORT}": job "check-and-lint" uses "ioBroker/testing-action-check@${lowerRefs.join(', ')}" ` +
        ...
        if (context.cfg.onlyWWW) {
            ...
            if (!jobHasStepWithAction(jobs['adapter-tests'], adapterActionPrefix)) {
                ...
                if (lowerRefs.length > 0) {
                    context.warnings.push(
                        `[W3029] Workflow "${WORKFLOW_FILE_SHORT}": job "adapter-tests" uses "ioBroker/testing-action-adapter@${lowerRefs.join(', ')}" ` +
        ...
        if (!jobs['deploy']) {
            ...
            if (!jobHasStepWithAction(jobs['deploy'], deployActionPrefix)) {
                ...
                if (lowerRefs.length > 0) {
                    context.warnings.push(
                        `[W3030] Workflow "${WORKFLOW_FILE_SHORT}": job "deploy" uses "ioBroker/testing-action-deploy@${lowerRefs.join(', ')}" ` +
