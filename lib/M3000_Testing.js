...
const { requestGithub } = require('./githubClient.js');
...
async function fetchJobLog(context, jobId) {
    ...
    try {
        ...
        const response = await requestGithub(url, { maxRedirects: 10, responseType: 'text' });
...
async function checkTests(context) {
    ...
    try {
        ...
        while (hasMore) {
            ...
            const workflowRunsResponse = await requestGithub(url);
        ...
        if (!Array.isArray(workflowRuns) || workflowRuns.length === 0) {
            ...
            const tagsResponse = await requestGithub(`${context.githubUrlApi}/tags?per_page=100`);
