...
/**
 * Returns true if the workflow run was created more than 90 days ago.
 * GitHub deletes workflow run logs after 90 days, causing 410 errors on retrieval.
 *
 * @param {object} run - GitHub Actions workflow run object (must have `created_at` field)
 * @returns {boolean} true if the run is older than 90 days
 */
function isRunOlderThan90Days(run) {
    // console.log(`### checking run ${run.id} + ${run.created_at}`);
...
async function checkTagTriggeredRun(context, workflowRuns, version, taggedVersion, codeBase, sourceLabel) {
    ...
    if (tagRun.conclusion === 'cancelled') {
        ...
        try {
            let oldWorkflowRun = false;
            // GitHub deletes workflow logs after 90 days (returns HTTP 410).
            // Instead of emitting one issue per job, emit a single suggestion for the whole run.
            if (isRunOlderThan90Days(tagRun)) {
                oldWorkflowRun = true;
                context.warnings.push(
                    `[S3056] "${WORKFLOW_FILE_SHORT}": workflow ${formatRunLink(tagRun)} for ${sourceLabel} is older than 90 days; log files might no longer be available.`,
                );
            }
...
function jobConditionUsesAlways(job) {
    return typeof job?.if === 'string' && /\balways\s*\(\s*\)/i.test(job.if);
}
...
async function fetchJobLog(context, jobId) {
    ...
    try {
        ...
        const response = await context.axios.get(url, { maxRedirects: 10, responseType: 'text' });
...
async function checkTests(context) {
    ...
    try {
        ...
        while (hasMore) {
            ...
            const workflowRunsResponse = await context.axios(url);
        ...
        if (!Array.isArray(workflowRuns) || workflowRuns.length === 0) {
            ...
            if (!Array.isArray(defaultBranchRuns) || defaultBranchRuns.length === 0) {
                ...
                try {
                    ...
                    // GitHub deletes workflow logs after 90 days (returns HTTP 410).
                    // Instead of emitting one issue per job, emit a single suggestion for the whole run.
                    if (isRunOlderThan90Days(lastRun)) {
                        context.warnings.push(
                            `[S3056] "${WORKFLOW_FILE_SHORT}": workflow ${formatRunLink(lastRun)} on branch "${defaultBranch}" is older than 90 days; log files are no longer available.`,
                        );
                        oldWorkflowRun = true;
                    }
            ...
            const tagsResponse = await context.axios(`${context.githubUrlApi}/tags?per_page=100`);
            ...
            try {
                ...
                if (!/^[a-z0-9][a-z0-9._-]*$/i.test(adapterName)) {
                    ...
                    const npmResponse = await context.axios(`https://registry.npmjs.org/${npmPackageName}`);
    ...
    if (!hasLegacyTesting) {
        ...
        for (const [jobName, job] of Object.entries(jobs)) {
            if (jobConditionUsesAlways(job)) {
                if (['check-and-lint', 'adapter-tests', 'deploy'].includes(jobName)) {
                    context.errors.push(
                        `[E3057] "${WORKFLOW_FILE_SHORT}": job "${jobName}" uses "always()" in its job condition. ` +
                            'Standard jobs must not use "always()"; remove it.',
                    );
                } else {
                    context.warnings.push(
                        `[S3058] "${WORKFLOW_FILE_SHORT}": job "${jobName}" uses "always()" in its job condition. ` +
                            'Please verify whether "always()" is really needed.',
                    );
                }
            }
        ...
        if (context.cfg.onlyWWW) {
            ...
            if (!jobHasStepWithAction(jobs['adapter-tests'], adapterActionPrefix)) {
                ...
                // W3059 / E3060: adapter-tests should not override engine-strict in testing-action-adapter
                const adapterStepWithEngineStrict = jobs['adapter-tests'].steps.find(
                    step =>
                        typeof step.uses === 'string' &&
                        step.uses.startsWith(adapterActionPrefix) &&
                        step.with &&
                        Object.prototype.hasOwnProperty.call(step.with, 'engine-strict'),
                );
                if (adapterStepWithEngineStrict) {
                    // engine-strict may be a boolean or a string in the workflow yaml
                    const engineStrictValue = `${adapterStepWithEngineStrict.with['engine-strict']}`.trim().toLowerCase();
                    if (engineStrictValue === 'false') {
                        context.errors.push(
                            `[E3060] "${WORKFLOW_FILE_SHORT}": job "adapter-tests" step using "ioBroker/testing-action-adapter" ` +
                                `sets "engine-strict" to false. Setting engine-strict to false is prohibited as this could hide errors occurring during installation at target systems.`,
                        );
                    } else {
                        context.warnings.push(
                            `[W3059] "${WORKFLOW_FILE_SHORT}": job "adapter-tests" step using "ioBroker/testing-action-adapter" ` +
                                `specifies "engine-strict". This parameter should not be changed; the default value should be used.`,
                        );
                    }
                }
            }
...
// [S3054] "${WORKFLOW_FILE_SHORT}": log for "check-and-lint" job in ${formatRunLink(tagRun)} for ${sourceLabel} does no longer exist.`,
// [S3055] "${WORKFLOW_FILE_SHORT}": log for "${adapterJob.name}" job in ${formatRunLink(tagRun)} for ${sourceLabel} does no longer exist.`,
// [S3056] "${WORKFLOW_FILE_SHORT}": workflow ${formatRunLink(run)} for ${sourceLabel} is older than 90 days; log files are no longer available.
// [E3057] "${WORKFLOW_FILE_SHORT}": job "${jobName}" uses "always()" in its job condition. Standard jobs must not use "always()".
// [S3058] "${WORKFLOW_FILE_SHORT}": job "${jobName}" uses "always()" in its job condition. Please verify whether "always()" is really needed.
