...
async function checkTests(context) {
    ...
    if (!hasLegacyTesting) {
        ...
        let enginesMajor = 0;
        const minimumNodeJsSupported = context.minimumNodeJsSupported || common.getMinimumNodeJsVersion(enginesNode);
        if (minimumNodeJsSupported) {
            enginesMajor = parseInt(minimumNodeJsSupported.split('.')[0], 10);
