            ...
            if (!checkAndLintJob) {
                ...
                if (!checkAndLintLog) {
                    ...
                    if (missingCheckLines.length > 0) {
                        context.warnings.push(
                            `[S3051] "${WORKFLOW_FILE_SHORT}": "testing-action-check" in ${formatRunLink(tagRun)} ` +
                    ...
                    for (const adapterJob of adapterTestJobs) {
                        ...
                        if (!adapterLog) {
                            ...
                            if (missingAdapterLines.length > 0) {
                                context.warnings.push(
                                    `[S3053] "${WORKFLOW_FILE_SHORT}": "testing-action-adapter" job "${adapterJob.name}" in ${formatRunLink(tagRun)} ` +
                ...
                try {
                    ...
                    if (!checkAndLintJob) {
                        ...
                        if (!checkAndLintLog) {
                            ...
                            if (missingCheckLines.length > 0) {
                                context.warnings.push(
                                    `[S3051] "${WORKFLOW_FILE_SHORT}": "testing-action-check" in ${formatRunLink(lastRun)} ` +
                                        `does not log the expected defaults for "npm run test:package". ` +
                                        `Please check the [log](${checkAndLintJob.html_url}),`,
                                );
                                if (common.isDebug()) {
                        ...
                        if (adapterTestJobs.length === 0) {
                            ...
                            for (const adapterJob of adapterTestJobs) {
                                ...
                                if (!adapterLog) {
                                    ...
                                    if (missingAdapterLines.length > 0) {
                                        context.warnings.push(
                                            `[S3053] "${WORKFLOW_FILE_SHORT}": "testing-action-adapter" job "${adapterJob.name}" in ${formatRunLink(lastRun)} ` +
                                                `does not log the expected defaults for "npm run test:integration". ` +
                                                `Please check the [log](${adapterJob.html_url}).`,
                                        );
                                        if (common.isDebug()) {
