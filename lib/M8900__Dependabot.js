...
async function checkDependabot(context) {
    ...
    // W8917: Check if npm entries covering root have an ignore block for @types/node semver-major updates
    if (hasNpm) {
        const npmEntries = updates.filter(u => u['package-ecosystem'] === 'npm');
        const rootNpmEntries = npmEntries.filter(u => {
            const dir = u['directory'];
            const dirs = u['directories'];
            const hasRootDir = dir === '/';
            const hasWildcardDirs = Array.isArray(dirs) && dirs.some(d => d === '**/*' || d === '**');
            return hasRootDir || hasWildcardDirs;
        });

        if (rootNpmEntries.length > 0) {
            const hasTypesNodeIgnore = rootNpmEntries.some(u => {
                const ignoreList = u['ignore'];
                if (!Array.isArray(ignoreList)) {
                    return false;
                }
                return ignoreList.some(entry => {
                    if (entry['dependency-name'] !== '@types/node') {
                        return false;
                    }
                    const updateTypes = entry['update-types'];
                    return Array.isArray(updateTypes) && updateTypes.includes('version-update:semver-major');
                });
            });

            if (!hasTypesNodeIgnore) {
                context.warnings.push(
                    `[W8917] At least major versions of "@types/node" should not be updated by dependabot. ` +
                        `Please add an ignore block to ".github/dependabot.yml":\n` +
                        `  ignore:\n` +
                        `    - dependency-name: "@types/node"\n` +
                        `      update-types: ["version-update:semver-major"]`,
                );
            } else {
                context.checks.push('Dependabot npm entry ignores major version updates for "@types/node".');
            }
        }
    }
...
// [W8916] Dependabot configuration "${DEPENDABOT_FILE}" has no entry with "package-ecosystem: github-actions". Please add one.
// [W8917] npm entry covering root has no ignore block for "@types/node" semver-major updates.
