...
const issuesToWatchAsWarning = [
    ...
    'Adapter published at ioBroker latest repository - Please create a tester topic at the forum',
    'Compatibility check and testing for node.js 26',
];

const common = require('./common.js');
const M8900__Dependabot = require('./M8900__Dependabot.js');

async function getAllIssues(context) {
    // console.log('\ngetAllIssues');
    // console.log(`${context.githubUrlApi}/issues`);
    const response = await context.axios(`${context.githubUrlApi}/issues`, {});
...
async function getCommitInfos(context) {
    ...
    try {
        ...
        response = await context.axios(url);
    } catch (e) {
        const message = common.getResponseMessage(e);
        common.error(`error downloading commit info from ${url} - ${e}${message ? ` - ${message}` : ''}`);
        throw e;
    }
...
async function checkGithubRepo(context) {
    ...
    let releases = [];
    try {
        const releasesResponse = await context.axios(`${context.githubUrlApi}/releases?per_page=20`);
        releases = Array.isArray(releasesResponse.data) ? releasesResponse.data : [];
    } catch (e) {
        const message = common.getResponseMessage(e);
        common.error(
            `error downloading release info from ${context.githubUrlApi}/releases - ${e}${message ? ` - ${message}` : ''}`,
        );
    }
    ...
    } else if (context.npmLatestVersion) {
        const latestGithubRelease =
            releases.find(release => !release.draft && !release.prerelease) ||
            releases.find(release => !release.draft) ||
            releases[0];
        const latestGithubTag = latestGithubRelease && latestGithubRelease.tag_name ? latestGithubRelease.tag_name : '';
        const normalizedGithubTag = latestGithubTag.replace(/^v/i, '');
        const normalizedNpmVersion = context.npmLatestVersion.replace(/^v/i, '');

        if (normalizedGithubTag !== normalizedNpmVersion) {
            context.warnings.push(
                `[W8005] Latest GitHub release tag (${latestGithubTag}) does not match latest npm version (${context.npmLatestVersion}).`,
            );
        } else {
            context.checks.push(
                `Latest GitHub release tag ${latestGithubTag} matches latest npm version ${context.npmLatestVersion}`,
            );
        }
    }
    ...
    // E8900 - E8999: reserved for M8900__Dependabot
    await M8900__Dependabot.checkDependabot(context);
...
// [W8005] Latest GitHub release tag does not match latest npm version.
// [S8005] No GitHub release found. Please use GitHub releases and consider using "@iobroker/action-deploy".
//
// [E8900 - E8999] Dependabot checks - see M8900__Dependabot.js
