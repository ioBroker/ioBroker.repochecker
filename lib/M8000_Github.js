...
async function getAllIssues(context) {
    ...
    const response = await requestGithub(`${context.githubUrlApi}/issues`);
...
async function checkGithubRepo(context) {
    ...
    let releases = [];
    try {
        const releasesResponse = await requestGithub(`${context.githubUrlApi}/releases?per_page=20`);
        releases = Array.isArray(releasesResponse.data) ? releasesResponse.data : [];
    } catch (e) {
        common.error(`error downloading release info from ${context.githubUrlApi}/releases - ${e}`);
    }
