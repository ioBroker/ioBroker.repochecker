...
async function checkEmailAddresses(context) {
    ...
    // Helper: validate one email source, remember valid addresses for further checks
    const checkSource = (email, sourceDescription, errorCode) => {
        if (!email || !isValidEmailFormat(email)) {
            context.errors.push(`[${errorCode}] No valid email address found for ${sourceDescription}`);
            return;
        }
        context.checks.push(`Valid email address found for ${sourceDescription}`);
        emailsToVerify.add(email.toLowerCase());
    };

    // 1) author of package.json
    checkSource(getEmailFromEntry(context.packageJson && context.packageJson.author), 'author in package.json', 'E4048');

    // 2) authors of io-package.json (use first entry providing an email)
    let ioAuthorEmail = null;
    const ioAuthors = context.ioPackageJson && context.ioPackageJson.common && context.ioPackageJson.common.authors;
    if (Array.isArray(ioAuthors)) {
        for (const entry of ioAuthors) {
            ioAuthorEmail = getEmailFromEntry(entry);
            if (ioAuthorEmail) {
                break;
            }
        }
    } else if (ioAuthors) {
        ioAuthorEmail = getEmailFromEntry(ioAuthors);
    }
    checkSource(ioAuthorEmail, 'author in io-package.json', 'E4049');

    // 2b) maintainers of package.json (validate every entry the same way as authors)
    ...
    if (Array.isArray(maintainers)) {
        for (const entry of maintainers) {
            checkSource(getEmailFromEntry(entry), 'maintainer in package.json', 'E4055');
        }
    } else if (maintainers) {
        checkSource(getEmailFromEntry(maintainers), 'maintainer in package.json', 'E4055');
    }

    // 3) copyright line at README.md
    try {
        const readme = await common.getFile(context, '/README.md');
        const readmeCopyright = (readme || '').split('\n').find(line => /copyright/i.test(line));
        checkSource(extractEmails(readmeCopyright)[0], 'copyright line in README.md', 'E4050');
    } catch {
        // README existence is checked elsewhere - do not raise a second error here
    }

    // 4) copyright line at LICENSE
    try {
        const license = await common.getFile(context, '/LICENSE');
        const licenseCopyright = (license || '').split('\n').find(line => /copyright/i.test(line));
        checkSource(extractEmails(licenseCopyright)[0], 'copyright line in LICENSE', 'E4051');
    } catch {
        // LICENSE existence is checked elsewhere - do not raise a second error here
    }
...
// [4048] No valid email address found for author in package.json
// [4049] No valid email address found for author in io-package.json
// [4050] No valid email address found for copyright line in README.md
// [4051] No valid email address found for copyright line in LICENSE
...
// [4055] No valid email address found for maintainer in package.json
