...
async function checkEmailAddresses(context) {
    ...
    // sources: array of { label, email }. If at least one source provides a valid email
    // the severity is reduced from error to warning, otherwise an error is raised.
    ...
    const checkGroup = (sources, areaDescription, errorCode) => {
        ...
        for (const source of sources) {
            if (source.email && isValidEmailFormat(source.email)) {
                hasValid = true;
                emailsToVerify.add(source.email.toLowerCase());
            } else {
    ...
    if (pkgAuthor) {
        personSources.push({ label: `package.json author "${describeEntry(pkgAuthor)}"`, email: getEmailFromEntry(pkgAuthor) });
    }
    ...
    for (const entry of ioAuthorsList) {
        personSources.push({ label: `io-package.json author "${describeEntry(entry)}"`, email: getEmailFromEntry(entry) });
    }
    ...
    for (const entry of maintainersList) {
        personSources.push({ label: `package.json maintainer "${describeEntry(entry)}"`, email: getEmailFromEntry(entry) });
    }
    ...
    const checkCopyrightFile = async (path, fileDescription, suggestionCode) => {
        ...
        for (const line of copyrightLines) {
            ...
            if (email && isValidEmailFormat(email)) {
                emailsToVerify.add(email.toLowerCase());
            } else {
    ...
    for (const email of emailsToVerify) {
        ...
        if (/noreply/i.test(email) && domain && domain.endsWith('github.com')) {
            context.errors.push(`[E4052] GitHub noreply email address "${email}" is not allowed. Please use a real email address.`);
            continue;
        }

        // 6) disallow known disposable / one time use email providers
        if (domain && DISPOSABLE_EMAIL_DOMAINS.includes(domain)) {
            context.warnings.push(`[W4053] Email address "${email}" uses a known disposable email provider. Please use a permanent email address.`);
            continue;
        }
        ...
        try {
            ...
            if (!records || !records.length) {
                context.warnings.push(`[S4054] Email address "${email}" could not be validated - no MX record found for domain "${domain}".`);
            } else {
...
// [4052] GitHub noreply email address "${email}" is not allowed. Please use a real email address.
// [4053] Email address "${email}" uses a known disposable email provider. Please use a permanent email address.
// [4054] Email address "${email}" could not be validated - no MX record found for domain "${domain}".
