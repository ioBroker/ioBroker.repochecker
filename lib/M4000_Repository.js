...
async function checkEmailAddresses(context) {
    ...
    const checkCopyrightFile = async (path, fileDescription, errorCode) => {
        let content;
        try {
        ...
        const sources = copyrightLines.map(line => ({ label: line.trim(), email: extractEmails(line)[0] }));
        checkGroup(sources, `copyright line in ${fileDescription}`, errorCode);
    };

    await checkCopyrightFile('/README.md', 'README.md', 'E4050');
    await checkCopyrightFile('/LICENSE', 'LICENSE', 'E4051');
...
// [4050] No valid email address found for copyright line in README.md${detail}
// [4051] No valid email address found for copyright line in LICENSE${detail}
