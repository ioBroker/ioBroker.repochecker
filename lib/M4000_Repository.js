...
async function checkEmailAddresses(context) {
    ...
    for (const [email, locations] of emailsToVerify) {
        ...
        if (/noreply/i.test(email) && domain && domain.endsWith('github.com')) {
            context.errors.push(`[E4052] GitHub noreply email address "${email}" (${at}) is not allowed. Please use a real email address.`);
            continue;
        }
...
// [4052] GitHub noreply email address "${email}" (${at}) is not allowed. Please use a real email address.
