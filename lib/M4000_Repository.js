...
async function getLatestRepo(context) {
    ...
    try {
        ...
        const message = common.getResponseMessage(e);
        common.error(`Cannot download latest repository info: ${e}${message ? ` - ${message}` : ''}`);
    }
    return context;
}
...
async function checkRepository(context) {
    ...
    if (context.ioPackageJson && context.ioPackageJson.common && context.ioPackageJson.common.type) {
        ...
        let _response = await context.axios(
        ...
        _response = await context.axios('https://download.iobroker.net/sources-dist-latest.json');
        body = _response.data;
        if (!body) {
            context.errors.push('[E4029] Cannot download https://download.iobroker.net/sources-dist-latest.json');
            throw 'Cannot download https://download.iobroker.net/sources-dist-latest.json'; // ABORT to avoid generating unuseable report
        } else {
            context.latestRepoLive = body;
        }

        _response = await context.axios('https://download.iobroker.net/sources-dist.json');
        body = _response.data;
        if (!body) {
            context.errors.push('[E4030] Cannot download https://download.iobroker.net/sources-dist.json');
            throw 'Cannot download https://download.iobroker.net/sources-dist.json'; // ABORT to avoid generating unuseable report
        } else {
    ...
    // Check email addresses used at author/maintainer/copyright entries
    await checkEmailAddresses(context);

    return context;
    // max number is E449
}

// Small list of well known disposable / one time use email providers.
// This list is intentionally kept short and can be extended if needed.
...
// Extract all email addresses contained in an arbitrary string.
// Matches a standard copyright notice line that carries a year specification,
// e.g. "Copyright (c) 2026 SentiQ <yves@nuesser.digital>" or a year range
// "Copyright (c) 2020-2026 ..." / "Copyright (c) 2020 - 2026 ...". The year
// specification prevents this from matching license boilerplate lines that merely
// contain the word "copyright" (e.g. the MIT "above copyright notice" sentence).
const COPYRIGHT_NOTICE_REGEX = /copyright\s*(?:\(c\)|©)?\s*\d{4}(?:\s*-\s*\d{4})?/i;

function extractEmails(text) {
    if (!text || typeof text !== 'string') {
        return [];
    }
    return text.match(/[^\s<>@()]+@[^\s<>@()]+\.[a-zA-Z]{2,}/g) || [];
}

// Validate the basic format of a single email address.
function isValidEmailFormat(email) {
    return typeof email === 'string' && /^[^\s<>@()]+@[^\s<>@()]+\.[a-zA-Z]{2,}$/.test(email);
}

// Read the first email address from an author/maintainer entry which may be a
// string ("Name <mail@example.com>") or an object ({ name, email }).
function getEmailFromEntry(entry) {
    ...
    if (typeof entry === 'string') {
        return extractEmails(entry)[0] || null;
    }
    return null;
}

// Build a short human readable label for an author/maintainer entry.
function describeEntry(entry) {
    ...
    if (typeof entry === 'object') {
        return entry.name || entry.email || '<empty>';
    }
    return '<empty>';
}

async function checkEmailAddresses(context) {
    const dns = require('dns').promises;
    // Map of email address -> set of locations where the address was found
    const emailsToVerify = new Map();
    const rememberEmail = (email, location) => {
        const key = email.toLowerCase();
        if (!emailsToVerify.has(key)) {
            emailsToVerify.set(key, new Set());
        }
        emailsToVerify.get(key).add(location);
    };

    // Validate a group of email sources and raise at most ONE log entry for the group.
    // sources: array of { label, email, location }. If at least one source provides a valid
    // email the severity is reduced from error to warning, otherwise an error is raised.
    // All sources with a missing or invalid address are listed within the single entry.
    const checkGroup = (sources, areaDescription, errorCode) => {
        const invalid = [];
        let hasValid = false;
        for (const source of sources) {
            if (source.email && isValidEmailFormat(source.email)) {
                hasValid = true;
                rememberEmail(source.email, source.location);
            } else {
                invalid.push(source.label);
            }
        }

        if (invalid.length === 0 && hasValid) {
            context.checks.push(`Valid email address found for ${areaDescription}`);
            return;
        }

        const detail = invalid.length ? ` (${invalid.join(', ')})` : '';
        const message = `[${errorCode}] No valid email address found for ${areaDescription}${detail}`;
        if (hasValid) {
            // at least one valid address exists in this group -> reduce to warning
    ...
    // 1) author / maintainer of package.json and io-package.json (checked as one group)
    const personSources = [];

    const pkgAuthor = context.packageJson && context.packageJson.author;
    if (pkgAuthor) {
        personSources.push({ label: `package.json author "${describeEntry(pkgAuthor)}"`, email: getEmailFromEntry(pkgAuthor), location: 'package.json' });
    }

    const ioAuthors = context.ioPackageJson && context.ioPackageJson.common && context.ioPackageJson.common.authors;
    const ioAuthorsList = Array.isArray(ioAuthors) ? ioAuthors : ioAuthors ? [ioAuthors] : [];
    for (const entry of ioAuthorsList) {
        personSources.push({ label: `io-package.json author "${describeEntry(entry)}"`, email: getEmailFromEntry(entry), location: 'io-package.json' });
    }

    const maintainers = context.packageJson && context.packageJson.maintainers;
    const maintainersList = Array.isArray(maintainers) ? maintainers : maintainers ? [maintainers] : [];
    for (const entry of maintainersList) {
        personSources.push({ label: `package.json maintainer "${describeEntry(entry)}"`, email: getEmailFromEntry(entry), location: 'package.json' });
    }

    checkGroup(personSources, 'author/maintainer', 'E4048');

    // 2) copyright lines at README.md and LICENSE (each file checked as one group).
    //    A missing email address is only a suggestion here and is raised to an
    //    error again for new adapters via lib/postprocessing.js.
    const checkCopyrightFile = async (path, fileDescription, suggestionCode) => {
        let content;
        try {
            content = await common.getFile(context, path);
        } catch {
            // file existence is checked elsewhere - do not raise a second error here
            return;
        }
        // Only look at actual copyright notice lines of the form
        //   "Copyright (c) <year> <name> <email>"
        // where <year> is a single year (nnnn) or a range (nnnn-mmmm / nnnn - mmmm).
        // Requiring the year specification avoids matching license boilerplate such
        // as "The above copyright notice and this permission notice shall be ...".
        const copyrightLines = (content || '').split('\n').filter(line => COPYRIGHT_NOTICE_REGEX.test(line));
        const invalid = [];
        for (const line of copyrightLines) {
            const email = extractEmails(line)[0];
            if (email && isValidEmailFormat(email)) {
                rememberEmail(email, fileDescription);
            } else {
                invalid.push(line.trim());
            }
        }

        if (copyrightLines.length && invalid.length === 0) {
            context.checks.push(`Valid email address found for copyright line in ${fileDescription}`);
            return;
        }

        const detail = invalid.length ? ` (${invalid.join(', ')})` : '';
        context.warnings.push(`[${suggestionCode}] No valid email address found for copyright line in ${fileDescription}${detail}`);
    };

    await checkCopyrightFile('/README.md', 'README.md', 'S4050');
    await checkCopyrightFile('/LICENSE', 'LICENSE', 'S4051');

    // Additional checks for each valid email address collected above
    for (const [email, locations] of emailsToVerify) {
        const domain = email.split('@')[1];
        const at = `at ${[...locations].join(', ')}`;

        // 5) disallow GitHub noreply addresses
        //    warning here; raised to an error again for new adapters via lib/postprocessing.js
        if (/noreply/i.test(email) && domain && domain.endsWith('github.com')) {
            context.warnings.push(`[W4052] GitHub noreply email address "${email}" (${at}) is not allowed. Please use a real email address.`);
            continue;
        }

        // 6) disallow known disposable / one time use email providers
        if (domain && DISPOSABLE_EMAIL_DOMAINS.includes(domain)) {
            context.warnings.push(`[W4053] Email address "${email}" (${at}) uses a known disposable email provider. Please use a permanent email address.`);
            continue;
        }

        // 7) try to validate that the domain can receive email (MX record lookup)
        try {
            const records = await dns.resolveMx(domain);
            if (!records || !records.length) {
                context.warnings.push(`[S4054] Email address "${email}" (${at}) could not be validated - no MX record found for domain "${domain}".`);
            } else {
                context.checks.push(`MX record found for email domain "${domain}"`);
            }
        } catch {
            context.warnings.push(`[S4054] Email address "${email}" (${at}) could not be validated - no MX record found for domain "${domain}".`);
        }
...
async function checkVSCodeSchemas(context) {
    try {
        ...
        // Find schema that matches any present jsonConfig files
...
// [4029] Cannot download https://download.iobroker.net/sources-dist-latest.json
// [4030] Cannot download https://download.iobroker.net/sources-dist.json
...
// [4048] No valid email address found for author/maintainer${detail}
// [4050] No valid email address found for copyright line in README.md${detail} (suggestion; raised to error for new adapters)
// [4051] No valid email address found for copyright line in LICENSE${detail} (suggestion; raised to error for new adapters)
// [4052] GitHub noreply email address "${email}" (${at}) is not allowed. Please use a real email address. (warning; raised to error for new adapters)
// [4053] Email address "${email}" (${at}) uses a known disposable email provider. Please use a permanent email address.
// [4054] Email address "${email}" (${at}) could not be validated - no MX record found for domain "${domain}".
