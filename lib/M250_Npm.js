    ...
    if (!body) {
        try {
            ...
            if (!_response.data) {
                ...
                if (!body.includes('href="/~bluefox"') && body.includes('href="/~iobluefox"')) {
                    context.errors.push(`[E251] Bluefox was not found in the collaborators on NPM!. Please execute in adapter directory: "npm owner add bluefox iobroker.${context.adapterName}"`);
                } else {
        ...
        if (!body['dist-tags'] ||
            ...
            if (compareVersions.compare(body['dist-tags'].latest, context.packageJson.version, '>=')) {
                context.errors.push(`[E253] Version of package.json (${context.packageJson.version}) lower than latest version on NPM (${(body['dist-tags'] && body['dist-tags'].latest) || JSON.stringify(body)})`);
            } else {
                context.warnings.push(`[W252] Version of package.json (${context.packageJson.version}) doesn't match latest version on NPM (${(body['dist-tags'] && body['dist-tags'].latest) || JSON.stringify(body)})`);
            }
        } else {
    ...
    if (context.ioPackageJson && context.ioPackageJson.common && context.ioPackageJson.common.news) {
        ...
        if (missingVersions.length) {
            if (missingVersions.length === 1) {
                context.errors.push(`[E254] Version "${missingVersions.join(', ')}" listed at common.news at io-package.json does not exist at NPM. Please remove from news section.`);
            } else {
                context.errors.push(`[E254] Versions "${missingVersions.join(', ')}" listed at common.news at io-package.json do not exist at NPM. Please remove from news section.`);
            }
        } else {
