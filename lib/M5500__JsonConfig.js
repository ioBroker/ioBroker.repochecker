...
// Mapping of component types to expected JavaScript types for their 'default' attribute.
// Components not listed here either don't use a default value or can hold mixed/arbitrary types.
const componentDefaultType = {
    checkbox: ['boolean'],
...
const validComponents = {
    ...
    checkDocker: { function: check_checkDocker, minAdmin: '7.7.31' },
    ...
    credential: { function: check_credential, minAdmin: '7.9.0' },
    ...
    iframe: { function: check_iframe, minAdmin: '7.7.28' },
    iframeSendTo: { function: check_iframeSendTo, minAdmin: '7.7.28' },
    ...
    oauth2: { function: check_oauth2, minAdmin: '7.8.9' },
    ...
    qrCodeSendTo: { function: check_qrCodeSendTo, minAdmin: '7.8.3' },
    ...
    yamlEditor: { function: check_yamlEditor, minAdmin: '7.7.31' },
};
...
function check_credential(context, path, jsonConfig, _inTable) {
    common.debug(`check_credential ${path}`);
    chk_responsive(context, path, jsonConfig, 'credential');
}
...
function check_iframe(context, path, jsonConfig, _inTable) {
    common.debug(`check_iframe ${path}`);
    chk_responsive(context, path, jsonConfig, 'iframe');
}

function check_iframeSendTo(context, path, jsonConfig, _inTable) {
    common.debug(`check_iframeSendTo ${path}`);
    chk_responsive(context, path, jsonConfig, 'iframeSendTo');
}
...
function check_oauth2(context, path, jsonConfig, _inTable) {
    common.debug(`check_oauth2 ${path}`);
    chk_responsive(context, path, jsonConfig, 'oauth2');
}
...
function check_qrCodeSendTo(context, path, jsonConfig, _inTable) {
    common.debug(`check_qrCodeSendTo ${path}`);
    chk_responsive(context, path, jsonConfig, 'qrCodeSendTo');
}
...
function check_yamlEditor(context, path, jsonConfig, _inTable) {
    common.debug(`check_yamlEditor ${path}`);
    chk_responsive(context, path, jsonConfig, 'yamlEditor');
}
...
function checkComponent(context, path, jsonConfig) {
    ...
    // Check that the 'default' value type matches the expected type for this component
    if (jsonConfig.default !== undefined && jsonConfig.default !== null && componentDefaultType[type] !== undefined) {
        const defaultType = Array.isArray(jsonConfig.default) ? 'array' : typeof jsonConfig.default;
        if (!componentDefaultType[type].includes(defaultType)) {
            context.warnings.push(
                `[W5516] type mismatch for "default" at ${path}: component "${type}" expects ${componentDefaultType[type].join(' or ')} but got ${defaultType}`,
            );
        }
    }
}
...
function checkTabs(context, path, jsonConfig) {
    ...
    for (const key in jsonConfig) {
        ...
        if (!type) {
            context.errors.push(`[E5505] no type specified at ${path}; expected "panel"`);
        }
        if (type !== 'panel') {
            context.errors.push(`[E5506] unexpected type "${key}" detected at ${path}; expected "panel"`);
        } else {
...
// Cached compiled AJV validator for jsonConfig schema (downloaded and compiled once per process)
let cachedJsonConfigValidate = null;

async function validateJsonConfigSchema(context, filePath, jsonConfigData) {
    common.debug(`[schema] validating ${filePath} against schema`);

    try {
        if (!cachedJsonConfigValidate) {
            const schemaUrl = config.schemaUrls.jsonConfig;
            common.debug(`Downloading schema from ${schemaUrl}`);

            const schemaData = await common.downloadURL(schemaUrl, false);
            if (!schemaData) {
                context.warnings.push(`[W5513] Could not download ${schemaUrl} for validation of ${filePath}`);
                return;
            }

            const schema = JSON.parse(schemaData);

            // Initialize AJV validator and cache the compiled schema
            common.debug(`parsing jsonConfig schema ${schemaUrl}`);
            // @ts-expect-error not constructable - but working
            const ajv = new Ajv({ allErrors: true, verbose: true, strictTypes: false, strictSchema: false });
            cachedJsonConfigValidate = ajv.compile(schema);
        }
    } catch (error) {
        common.debug(`Schema compilation error: ${error.message}`);
        context.warnings.push(`[W5515] schema compilation failed: ${error.message}`);
        return;
    }

    try {
        // Validate jsonConfig against schema
        const valid = cachedJsonConfigValidate(jsonConfigData);

        if (valid) {
            context.checks.push(`${filePath} validates against schema`);
        } else {
            // Log individual validation errors
            if (cachedJsonConfigValidate.errors) {
                for (const error of cachedJsonConfigValidate.errors) {
                    ...
                    if (error.keyword === 'if') {
                        ...
                        errorMsg = `Missing required property "${error.params.missingProperty}" in ${path || filePath}`;
                    ...
                    if (errorMsg !== '') {
                        context.warnings.push(`[W5512] ${filePath} schema validation error: ${errorMsg}`);
                    }
                }
            } else {
                context.warnings.push(`[W5512] ${filePath} does not validate against schema`);
            }
        }
    } catch (error) {
        common.debug(`Schema validation error: ${error.message}`);
        context.warnings.push(`[W5514] ${filePath} schema validation failed: ${error.message}`);
    }
}

exports.checkJsonConfig = checkJsonConfig;
exports.validateJsonConfigSchema = validateJsonConfigSchema;
...
// [5505] no type specified at ${path}; expected "panel"
// [5506] unexpected type "${key}" detected at ${path}; expected "panel"
...
// [E5512] ${filePath} schema validation error: ${errorMsg}
// [W5513] Could not download schema for validation of ${filePath}
// [E5514] ${filePath} schema validation failed: ${error.message}
// [W5515] schema compilation failed: ${error.message}
// [W5516] type mismatch for "default" at ${path}: component "${type}" expects ${expectedType} but got ${actualType}
