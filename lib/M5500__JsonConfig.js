        ...
        if (valid) {
            ...
            if (cachedJsonConfigValidate.errors) {
                for (const error of cachedJsonConfigValidate.errors) {
                    ...
                    context.errors.push(`[E5512] ${filePath} schema validation error: ${errorMsg}`);
                }
            } else {
                context.errors.push(`[E5512] ${filePath} does not validate against schema`);
            }
        }
    } catch (error) {
        common.debug(`Schema validation error: ${error.message}`);
        context.errors.push(`[E5514] ${filePath} schema validation failed: ${error.message}`);
    }
}
