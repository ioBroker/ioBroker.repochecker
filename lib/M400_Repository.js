...
async function checkVSCodeSchemas(context) {
    ...
    try {
        ...
        const hasJson5Files = await checkForJson5Files(context);

        // Find schema that matches jsonConfig files
        const jsonConfigSchema = schemas.find(
            schema => schema.fileMatch && jsonConfigFiles.some(file => schema.fileMatch.includes(file)),
        );
        ...
        const json5ConfigSchema = schemas.find(
            schema => schema.fileMatch && json5ConfigFiles.some(file => schema.fileMatch.includes(file)),
        );
...
async function checkForJson5Files(context) {
    try {
        const filesList = await common.getFilesList(context);
        return filesList.some(file => file.startsWith('/admin/') && file.endsWith('.json5'));
    } catch (error) {
        common.debug(`Could not check for JSON5 files: ${error.message}`);
        return false;
    }
}
