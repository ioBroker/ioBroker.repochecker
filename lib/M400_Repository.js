...
async function checkVSCodeSchemas(context) {
    ...
    try {
        ...
        try {
            ...
            context.errors.push('[E437] .vscode/settings.json file contains invalid JSON, please fix.');
            return;
        }
        ...
        if (!settingsJson['json.schemas']) {
            context.warnings.push(
                '[W438] .vscode/settings.json requires "json.schemas" property which must be an array. Please add schema definitions for better IntelliSense support.',
            );
            return;
        }
        ...
        if ((!ioPackageSchema) || (ioPackageSchema.url !== config.schemaUrls['io-package'])) {
            context.warnings.push(
                `[W440] Missing or incorrect schema definition for "io-package.json" in .vscode/settings.json. Add: {"fileMatch": ["io-package.json"], "url": "${config.schemaUrls['io-package']}"}`,
            );
#### WORK IN PROGRESS '''
        } else {
