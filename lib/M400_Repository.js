...
async function checkVSCodeSchemas(context) {
    try {
        ...
        if (!settingsJson['json.schemas']) {
            context.warnings.push(
                '[S438] .vscode/settings.json file missing "json.schemas" property. Add schema definitions for better IntelliSense support.',
            );
            return;
        }
        ...
        if (!json5ConfigSchema) {
            if (context.cfg.usesJson5) {
                context.warnings.push(
                    `[W444] Missing schema definition for JSON5 config files in .vscode/settings.json. Adapt: {"fileMatch": ${JSON.stringify(jsonConfigFiles.concat(json5ConfigFiles))}, "url": "${config.schemaUrls.jsonConfig}"}`,
                );
            }
        } else {
            // Check if URL is correct
