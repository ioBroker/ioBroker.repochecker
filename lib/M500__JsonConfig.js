...
function handleResponsiveWarning(context, msg) {
    if (responsiveWarningLimit) {
        ...
        if (responsiveWarningLimit === 0) {
            context.warnings.push(`[W510] responsive check: maximum issues reached, please fix reported ones and recheck`);
        }
    }
}
...
function chk_responsive(context, path, jsonConfig, component, opts) {
    ...
    if (!opts || !opts.sizesOptional) {
        ...
        if (missing.length) {
            handleResponsiveError(context, `[E507] missing size attributes [${missing.join(',')}] for ${component} at ${path}`);
        }
    }

    if (jsonConfig.xs && jsonConfig.xs != 12) {
        handleResponsiveWarning(context, `[W508] attribute "xs" for ${component} should specify a value of "12" at ${path}`);
    }

    let ii = 12;
    if (jsonConfig.xs) ii = jsonConfig.xs;
    if (jsonConfig.sm && jsonConfig.sm > ii) {
        handleResponsiveError(context, `[E509] "sm" value (${jsonConfig.sm}) for ${component} greater then value for smaller displays`);
    } else {
        ii = jsonConfig.sm;
    }
    if (jsonConfig.md && jsonConfig.md > ii) {
        handleResponsiveError(context, `[E509] "md" value (${jsonConfig.md}) for ${component} greater then value for smaller displays`);
    } else {
        ii = jsonConfig.md;
    }
    if (jsonConfig.lg && jsonConfig.lg > ii) {
        handleResponsiveError(context, `[E509] "lg" value (${jsonConfig.lg}) for ${component} greater then value for smaller displays`);
    } else {
        ii = jsonConfig.lg;
    }
    if (jsonConfig.xl && jsonConfig.xl > ii) {
        handleResponsiveError(context, `[E509] "xl" value (${jsonConfig.xl}) for ${component} greater then value for smaller displays`);
    }
}
    ...
    for (const key in jsonConfig) {
        ...
        if (key === 'type') {
            if (jsonConfig[key] !== 'panel') {
                common.log(`INTERNAL ERROR: unexpected type ${jsonConfig[key]} encountered while scanning "panel"`);
                return;
            }
        }
