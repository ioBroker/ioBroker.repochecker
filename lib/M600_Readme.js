...
async function checkReadme(context) {
    ...
    if (!data) {
        ...
        if (pos === -1) {
            ...
            common.debug(`README year ${readmeYear}`);
            common.debug(`Current year ${year}`);
            common.debug(`Commit year ${commitYear}`);
            common.debug(`NPM year ${npmYear}`);
            ...
            if (readmeYear > year) {
                ...
                if (m) {
                    context.errors.push(
                        `[E607] Future year (${readmeYear}) found in copyright. Please use "Copyright (c) ${m[1]}-${year} ${getAuthor(context.packageJson.author)}" at the end of README.md`,
                    );
                } else {
                    context.errors.push(
                        `[E607] Future year (${readmeYear}) found in copyright. Please use "Copyright (c) ${year} ${getAuthor(context.packageJson.author)}" at the end of README.md`,
                    );
                }
            } else if (!valid) {
