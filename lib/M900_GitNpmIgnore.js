        ...
        if (!tooComplexToCheck) {
            ...
            if (!isIgnored) {
                ...
                context.warnings.push(
                    '[S9006] .commitinfo file should be excluded by .gitignore, consider adding it to .gitignore',
                );
            }
        }
    }
...
async function checkNpmIgnore(context) {
    ...
    if (context.packageJson.files && context.packageJson.files.length) {
        if (context.filesList.includes('/.npmignore')) {
            context.warnings.push(
                `[W9051] .npmignore found but "files" is used at package.json. Please remove .npmignore.`,
            );
        } else {
    ...
    if (!context.filesList.includes('/.npmignore')) {
        context.errors.push(
            `[E9052] neither files section at package.json nor file .npmignore found. npm package will contain unwanted files.`,
        );
    } else {
        context.warnings.push(`[W9053] .npmignore found - consider using package.json object "files" instead.`);
        ...
        /*if (!rules.includes('node_modules') && !rules.includes('/node_modules') && !rules.includes('/node_modules/*') && !rules.includes('node_modules/*')) {
            !check && context.errors.push(`[E9054] node_modules not found in `.npmignore``);
        }*/
        if (!tooComplexToCheck) {
            // if (!rules.includes('iob_npm.done') && !rules.includes('/iob_npm.done')) {
            //     /*!check &&*/ context.errors.push(`[E9055] iob_npm.done not found in .npmignore`);
            // }
...
// [W9000]6] .commitinfo file should be excluded by .gitignore, consider adding it to .gitignore
