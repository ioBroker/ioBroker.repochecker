...
async function checkAutomerge(context) {
    ...
    if (!automergeConfigured) {
        ...
        context.warnings.push(
            `[S8913] No automerge workflow found. Consider setting up automerging by adding ".github/workflows/automerge-dependabot.yml".`,
        );
        return;
    }
...
async function checkDependabot(context) {
    ...
    for (const update of updates) {
        ...
        // S8906: Suggest cron instead of monthly
        if (interval === 'monthly') {
            context.warnings.push(
                `[S8906] Dependabot entry for "${ecosystem}" uses "schedule: interval: monthly". ` +
