...
async function checkCode(context) {
    ...
    if (context.ioPackageJson.common.adminUI && context.ioPackageJson.common.adminUI.config === 'json') {
        ...
        const w1073Keys = new Set(Array.isArray(context.w1073Keys) ? context.w1073Keys : []);
        const w1074Keys = new Set(Array.isArray(context.w1074Keys) ? context.w1074Keys : []);
        ...
        for (const [nativeKey] of passwordLocations.entries()) {
            if (!protectedNative.has(nativeKey) && !w1073Keys.has(nativeKey)) {
                missingProtected.push(nativeKey);
            }
            if (!encryptedNative.has(nativeKey) && !w1074Keys.has(nativeKey)) {
