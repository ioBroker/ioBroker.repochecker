...
async function checkCode(context) {
    ...
    if (context.ioPackageJson.common.adminUI && context.ioPackageJson.common.adminUI.config === 'json') {
        ...
        if (missingProtected.length) {
            context.warnings.push(
                `[W5057] jsonConfig password field "${missingProtected.join(', ')}" is not listed at "protectedNative" in io-package.json`,
            );
        }
        if (missingEncrypted.length) {
            context.warnings.push(
                `[W5058] jsonConfig password field "${missingEncrypted.join(', ')}" is not listed at "encryptedNative" in io-package.json`,
            );
        }
    }
...
// [W5057] jsonConfig password field "${nativeKey1, nativeKey2}" is not listed at "protectedNative" in io-package.json
// [W5058] jsonConfig password field "${nativeKey1, nativeKey2}" is not listed at "encryptedNative" in io-package.json
