        ...
        if (compareVersions.compareVersions(version, '3.0.0') >= 0) {
            if (!context['/.releaseconfig.json']) {
                ...
                try {
                    ...
                    // Check that all plugins listed in .releaseconfig.json are installed as devDependencies
                    if (plugins && Array.isArray(plugins)) {
                        for (const plugin of plugins) {
                            const pluginPackageName = `@alcalzone/release-script-plugin-${plugin}`;
                            if (!context.packageJson.devDependencies[pluginPackageName]) {
                                context.errors.push(
                                    `[E5034] Plugin "${plugin}" is listed in .releaseconfig.json but "${pluginPackageName}" is not installed as devDependency. Please add.`,
                                );
                            }
                        }
                    }
...
// [5034] Plugin "${plugin}" is listed in .releaseconfig.json but "${pluginPackageName}" is not installed as devDependency. Please add.
