...
async function checkCode(context) {
    ...
    // Check all JSON and JSON5 files for human-readable formatting
    for (const fileName of Object.keys(context)) {
        if (fileName.startsWith('/') && typeof context[fileName] === 'string') {
            const ext = path.extname(fileName);
            if (jsonExtensions.includes(ext)) {
    ...
    if (
        ...
        context.warnings.push(
            '[W5064] Using admin/jsonTab.json(5) requires common.adminUI to be set to "common.adminUI": { "tab": "json" }.',
        );
    }
...
// [W5064] "admin/jsonTab.json" or "admin/jsonTab.json5" found but "common.adminUI.tab" is not set to "json" in io-package.json. Please add "common.adminUI": { "tab": "json" }.
