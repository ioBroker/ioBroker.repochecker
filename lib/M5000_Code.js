...
async function checkCode(context) {
    ...
    // Track files containing sleep/wait function definitions for S5050 check
    const sleepWaitFiles = [];
    ...
    for (const filePath of Object.keys(context)) {
        ...
        // S5050: Check for sleep/wait function definitions (suggest using this.delay instead)
        // Matches: function sleep/wait(...), async function sleep/wait(...),
        //          const/let/var sleep/wait = [async] function/arrow
        if (
            /\b(?:async\s+)?function\s+(?:sleep|wait)\s*\(/.test(contentWithoutComments) ||
            /\b(?:const|let|var)\s+(?:sleep|wait)\s*=\s*(?:async\s*)?(?:function\b|\()/.test(contentWithoutComments)
        ) {
            sleepWaitFiles.push(filePath.slice(1));
        }
    ...
    // S5050: Report sleep/wait function definitions
    if (sleepWaitFiles.length > 0) {
        const fileList = sleepWaitFiles.join(', ');
        context.warnings.push(
            `[S5050] A custom "sleep" or "wait" function is implemented in source files (${fileList}). Consider using the standard "this.delay()" method provided by ioBroker instead.`,
        );
    }
