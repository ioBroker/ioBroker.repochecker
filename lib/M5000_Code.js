...
async function addFilesList(context, readFiles) {
    for (const fileName of context.filesList) {
        common.debug(`checking file ${fileName}`);

        const found = [];
        if (!common.isLocal() && fileName.match(/^\/node_modules\//) && !found.includes('node_module')) {
...
async function checkCode(context) {
    ...
    await addFilesList(context, readFiles);
