...
/**
 * Extract optional dependencies declared via repochecker line comments.
 *
 * Supported syntax (line comment only):
 *   // @repochecker: optional dependency 'pkg'
 *   // @repochecker: optional dependency "pkg"
 *
 * @param {string} content - File content
 * @returns {string[]} Array of unique optional dependency names
 */
function extractOptionalDependencies(content) {
    const optionalDependencies = new Set();
    const optionalDependencyPattern = /^\s*\/\/\s*@repochecker:\s*optional dependency\s+(['"])([^'"\r\n]+)\1\s*$/gm;
    let match;
    while ((match = optionalDependencyPattern.exec(content)) !== null) {
        optionalDependencies.add(match[2]);
    }
...
async function checkCode(context) {
    ...
    // react detection has been moved to config.updateConfig(), result is stored at context.usesReact
    if (
        !context.usesReact &&
    ...
    if (context.ioPackageJson.common.adminUI && context.ioPackageJson.common.adminUI.config === 'json') {
        ...
        if (jsonConfig) {
            ...
            if (i18nAttr === true || typeof i18nAttr === 'string') {
                ...
                context.warnings.push(`[W5015] Why did you decide to disable i18n support at jsonConfig.json(5)?`);
            } else {
                context.warnings.push(`[S5022] Consider adding explicit i18n attribute to jsonConfig.json(5)`);
            }
    ...
    if (context.ioPackageJson.common.adminUI && context.ioPackageJson.common.adminUI.config === 'json') {
        ...
        const reportedProtectedKeys = new Set(
            Array.isArray(context.reportedMissingProtectedNativeKeys) ? context.reportedMissingProtectedNativeKeys : [],
        );
        const reportedEncryptedKeys = new Set(
            Array.isArray(context.reportedMissingEncryptedNativeKeys) ? context.reportedMissingEncryptedNativeKeys : [],
        );
        ...
        for (const [nativeKey] of passwordLocations.entries()) {
            if (!protectedNative.has(nativeKey) && !reportedProtectedKeys.has(nativeKey)) {
                missingProtected.push(nativeKey);
            }
            if (!encryptedNative.has(nativeKey) && !reportedEncryptedKeys.has(nativeKey)) {
                missingEncrypted.push(nativeKey);
            }
        }

        if (missingProtected.length) {
            context.warnings.push(
                `[W5057] jsonConfig password field(s) "${missingProtected.join(', ')}" is not listed at "protectedNative" in io-package.json`,
            );
        }
        if (missingEncrypted.length) {
            context.warnings.push(
                `[W5058] jsonConfig password field(s) "${missingEncrypted.join(', ')}" is not listed at "encryptedNative" in io-package.json`,
            );
        }
    }
    ...
    // Check for outdated lib/tools.js (also .mjs and .cjs variants)
    const toolsFileExtensions = ['.js', '.mjs', '.cjs'];
    for (const toolsExt of toolsFileExtensions) {
        const toolsFile = `lib/tools${toolsExt}`;
        if (context.filesList.includes(`/${toolsFile}`)) {
            // Check if lib/tools file is used anywhere in the repository
            let isUsed = false;

            // Search through all loaded files for references to tools
            for (const fileName of Object.keys(context)) {
                if (fileName.startsWith('/') && typeof context[fileName] === 'string') {
                    const content = context[fileName];
                    // Look for various patterns that might reference lib/tools
                    if (
            ...
            if (!isUsed) {
                context.warnings.push(
                    `[S5032] "${toolsFile}" found in repo but not used anywhere. Consider removing file.`,
                );
            }
        }
    }
    ...
    // Packages provided transitively by "@iobroker/testing" (>= 5.1.1). This list is prepared by
    // M0000_PackageJson (stored into context.redundantTestingPackages) and only present when a
    // matching "@iobroker/testing" version is installed. When present these packages (e.g. sinon,
    // chai, mocha) do not need to be listed as (dev)dependencies, so W5042 must not be raised for them.
    const testingProvidedPackages = new Set(context.redundantTestingPackages || []);
    ...
    // '/path' entries match repository root directories only, while entries without '/'
    // match directory names at any level in the repository tree.
    const excludedSourceDirs = new Set([
        ...
        '/examples',
        '/scripts',
        // '/src/scripts', - do NOT include, might contain called code
        // '/src/lib/scripts', - do NOT include, might contain called code
        '/template',
        '/templates',
        '/test',
        '/tools',
    ...
    const excludedSourceFiles = new Set(['tasks.js', 'tasks.mjs', 'tasks.cjs', 'tasks.ts', 'Gruntfile.js', 'gulpfile.js']);

    // Exact relative file paths (without leading slash) to exclude from dependency scanning.
    // lib/tools.* is normally excluded, but if another source file imports (requires) it,
    // it must be scanned like any other source file.
    const excludedSourceRelPaths = new Set();
    const toolsRelPaths = ['lib/tools.js', 'lib/tools.mjs', 'lib/tools.cjs'];
    for (const toolsRelPath of toolsRelPaths) {
        let isImported = false;
        // Search all other loaded source files for a reference to lib/tools
        for (const fileName of Object.keys(context)) {
            if (fileName === `/${toolsRelPath}` || !fileName.startsWith('/') || typeof context[fileName] !== 'string') {
                continue;
            }
            const content = context[fileName];
            if (
        ...
        if (!isImported) {
            excludedSourceRelPaths.add(toolsRelPath);
        }
    }
    ...
    const usedImportedPackages = new Set();
    const implicitRuntimeDependencies = new Set();
    const optionalDependencyComments = new Set();
    const usedOptionalDependencies = new Set();
    ...
    for (const filePath of Object.keys(context)) {
        ...
        const optionalDependencies = extractOptionalDependencies(context[filePath]);
        for (const optionalDependency of optionalDependencies) {
            optionalDependencyComments.add(optionalDependency);
        }
        ...
        for (const importTarget of imports) {
            ...
            usedImportedPackages.add(packageName);
            ...
            // Skip packages provided transitively by "@iobroker/testing" (>= 5.1.1)
            if (testingProvidedPackages.has(packageName)) {
                continue;
            }
            ...
            } else if (optionalDependencyComments.has(packageName)) {
                usedOptionalDependencies.add(packageName);
                continue;
            } else {
                context.warnings.push(
    ...
    const formatSuggestionFileList = files =>
        files.length > 2 ? `${files.slice(0, 2).join(', ')}, ...` : files.join(', ');

    // S5004/W5004: Report bare setInterval() usage and missing clearInterval() separately
    if (setIntervalFiles.length > 0) {
        const fileList = formatSuggestionFileList(setIntervalFiles);
        context.warnings.push(
    ...
    if (setTimeoutFiles.length > 0) {
        const fileList = formatSuggestionFileList(setTimeoutFiles);
        context.warnings.push(
    ...
    for (const optionalDependency of optionalDependencyComments) {
        if (packageDependencies[optionalDependency]) {
            context.warnings.push(
                `[W5064] Optional dependency comment for package "${optionalDependency}" is invalid because the package is already listed in dependencies of package.json.`,
            );
            continue;
        }
        if (!usedImportedPackages.has(optionalDependency)) {
            context.warnings.push(
                `[W5065] Optional dependency comment for package "${optionalDependency}" is ignored because the package is not imported or required by any scanned source file.`,
            );
            continue;
        }
        if (usedOptionalDependencies.has(optionalDependency)) {
            context.warnings.push(
                `[S5066] Package "${optionalDependency}" is declared as an optional dependency via source comment and is used in source files, but is not listed in dependencies of package.json. It might be missing on target systems.`,
            );
        }
    }
...
// [5032] "lib/tools.js" (also .mjs/.cjs) found in repo but not used anywhere. Consider removing this outdated file.
...
// [W5057] jsonConfig password field(s) "${nativeKey1, nativeKey2}" is not listed at "protectedNative" in io-package.json
// [W5058] jsonConfig password field(s) "${nativeKey1, nativeKey2}" is not listed at "encryptedNative" in io-package.json
...
// [W5064] Optional dependency comment for package "${packageName}" is invalid because the package is already listed in dependencies of package.json.
// [W5065] Optional dependency comment for package "${packageName}" is ignored because the package is not imported or required by any scanned source file.
// [S5066] Package "${packageName}" is declared as an optional dependency via source comment and is used in source files, but is not listed in dependencies of package.json. It might be missing on target systems.
