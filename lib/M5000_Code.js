...
async function checkCode(context) {
    ...
    if (common.isLocal()) {
        ...
        tempFilesList = getAllFiles(context, process.cwd(), '', []);
    } else {
    ...
    // Check for setInterval/setTimeout usage in all executable files
    // Track files with issues to avoid duplicate reports
    const filesWithSetInterval = new Set();
    const filesWithSetTimeout = new Set();

    // Search through all loaded executable files for setInterval/setTimeout usage
    for (const fileName of Object.keys(context)) {
        if (fileName.startsWith('/') && typeof context[fileName] === 'string') {
            ...
            // Only check executable files (.js, .ts, .mjs, .cjs)
            const ext = path.extname(fileName);
            const executableExtensions = ['.js', '.mjs', '.cjs', '.ts'];
            if (!executableExtensions.includes(ext)) {
                continue;
            ...
            let suggestSetIntervalContext = null;
            let suggestSetTimeoutContext = null;

            for (let i = 0; i < lines.length; i++) {
                const line = lines[i];

                // Check for plain setInterval (not this. or adapter.)
                if (/\bsetInterval\s*\(/.test(line) && !/\b(this|adapter)\.setInterval\s*\(/.test(line)) {
                    hasSetInterval = true;
                    if (!suggestSetIntervalContext) {
                        // Determine if we should suggest this. or adapter.
                        suggestSetIntervalContext = isInAdapterClassContext(lines, i) ? 'this' : 'adapter';
                    }
                }

                // Check for plain setTimeout (not this. or adapter.)
                if (/\bsetTimeout\s*\(/.test(line) && !/\b(this|adapter)\.setTimeout\s*\(/.test(line)) {
                    hasSetTimeout = true;
                    if (!suggestSetTimeoutContext) {
                        // Determine if we should suggest this. or adapter.
                        suggestSetTimeoutContext = isInAdapterClassContext(lines, i) ? 'this' : 'adapter';
                    }
                }

                // Check for clearInterval (including this.clearInterval and adapter.clearInterval)
                if (/\bclearInterval\s*\(/.test(line)) {
                    hasClearInterval = true;
                }

                // Check for clearTimeout (including this.clearTimeout and adapter.clearTimeout)
                if (/\bclearTimeout\s*\(/.test(line)) {
                    hasClearTimeout = true;
                }
            }

            // Report setInterval issues
            if (hasSetInterval && !hasClearInterval && !filesWithSetInterval.has(fileName)) {
                filesWithSetInterval.add(fileName);
                if (context.ioPackageJson.common.compact) {
                    // if compact mode supported, it is critical
                    context.errors.push(
                        `[E5004] setInterval found in "${fileName}", but no clearInterval detected. Use ${suggestSetIntervalContext}.setInterval() instead.`,
                    );
                } else {
                    context.warnings.push(
                        `[W5004] setInterval found in "${fileName}", but no clearInterval detected. Consider using ${suggestSetIntervalContext}.setInterval() instead.`,
                    );
                }
            } else if (hasSetInterval && hasClearInterval && !filesWithSetInterval.has(fileName)) {
                // Suggest using this./adapter.setInterval even if clearInterval exists
                filesWithSetInterval.add(fileName);
                context.warnings.push(
                    `[S5004] Consider using ${suggestSetIntervalContext}.setInterval() instead of plain setInterval() in "${fileName}".`,
                );
            }

            // Report setTimeout issues
            if (hasSetTimeout && !hasClearTimeout && !filesWithSetTimeout.has(fileName)) {
                filesWithSetTimeout.add(fileName);
                // Note: setTimeout with this./adapter. should be error if compact is set, but this is currently disabled
                // For now, we'll warn for plain setTimeout without clearTimeout
                if (context.ioPackageJson.common.compact) {
                    // if compact mode supported, it is critical
                    context.warnings.push(
                        `[W5005] setTimeout found in "${fileName}", but no clearTimeout detected. Use ${suggestSetTimeoutContext}.setTimeout() instead.`,
                    );
                } else {
                    context.warnings.push(
                        `[W5005] setTimeout found in "${fileName}", but no clearTimeout detected. Consider using ${suggestSetTimeoutContext}.setTimeout() instead.`,
                    );
                }
            } else if (hasSetTimeout && hasClearTimeout && !filesWithSetTimeout.has(fileName)) {
                // Suggest using this./adapter.setTimeout even if clearTimeout exists
                filesWithSetTimeout.add(fileName);
                context.warnings.push(
                    `[S5005] Consider using ${suggestSetTimeoutContext}.setTimeout() instead of plain setTimeout() in "${fileName}".`,
                );
            }
        }
    }
...
// [5004] setInterval found in "${fileName}", but no clearInterval detected. Use ${context}.setInterval() instead.
// [5004] setInterval found in "${fileName}", but no clearInterval detected. Consider using ${context}.setInterval() instead.
// [5004] Consider using ${context}.setInterval() instead of plain setInterval() in "${fileName}".
// [5005] setTimeout found in "${fileName}", but no clearTimeout detected. Use ${context}.setTimeout() instead.
// [5005] setTimeout found in "${fileName}", but no clearTimeout detected. Consider using ${context}.setTimeout() instead.
// [5005] Consider using ${context}.setTimeout() instead of plain setTimeout() in "${fileName}".
