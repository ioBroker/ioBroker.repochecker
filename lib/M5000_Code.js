        ...
        dirFiles.forEach(files => {
            if (files.length > 1) {
                const filePaths = files
                    ...
                    .join(', ');
                context.errors.push(
                    `[E5034] Files with conflicting extensions found in directory "${dir}": ${filePaths} - Only one JavaScript file extension (.js, .cjs, or .mjs) should be used per file name.`,
                );
            }
        });
    });
