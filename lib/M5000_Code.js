...
async function checkCode(context) {
    ...
    // M0000_PackageJson (stored into context.redundantPackages) and only present when a matching
    // "@iobroker/testing" version is installed. When present these packages (e.g. sinon, chai, mocha)
    // do not need to be listed as (dev)dependencies, so W5042 must not be raised for them.
    const testingProvidedPackages = new Set(context.redundantPackages || []);
