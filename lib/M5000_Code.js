...
async function checkCode(context) {
    ...
    if (
        ...
        context.ioPackageJson.common.adminUI &&
        context.ioPackageJson.common.adminUI.config === 'materialize'
    ) {
        context.errors.push(
    ...
    // Track files with bare setInterval() for E5004/W5004 check (not this.setInterval or adapter.setInterval)
    const setIntervalFiles = [];

    // Track files with bare setTimeout() for W5005 check (not this.setTimeout or adapter.setTimeout)
    ...
    for (const filePath of Object.keys(context)) {
        ...
        // E5004/W5004: Check for bare setInterval() — negative lookbehind excludes this.setInterval / adapter.setInterval
        if (/(?<![.\w])setInterval\s*\(/.test(contentWithoutComments)) {
            const hasClearInterval = /clearInterval\s*\(/.test(contentWithoutComments);
            setIntervalFiles.push({ file: filePath.slice(1), hasClear: hasClearInterval });
        }

        // W5005: Check for bare setTimeout() — negative lookbehind excludes this.setTimeout / adapter.setTimeout
        if (/(?<![.\w])setTimeout\s*\(/.test(contentWithoutComments)) {
            const hasClearTimeout = /clearTimeout\s*\(/.test(contentWithoutComments);
            setTimeoutFiles.push({ file: filePath.slice(1), hasClear: hasClearTimeout });
        }
    ...
    // E5004/W5004: Report bare setInterval() usage across all source files
    for (const { file, hasClear } of setIntervalFiles) {
        const clearNote = hasClear ? '' : ' No clearInterval() detected.';
        const msg = `Plain setInterval() found in "${file}". Please use this.setInterval() or adapter.setInterval() instead.${clearNote}`;
        if (context.ioPackageJson.common.compact) {
            context.errors.push(`[E5004] ${msg}`);
        } else {
            context.warnings.push(`[W5004] ${msg}`);
        }
    }

    // W5005: Report bare setTimeout() usage across all source files
    for (const { file, hasClear } of setTimeoutFiles) {
        const clearNote = hasClear ? '' : ' No clearTimeout() detected.';
        context.warnings.push(
            `[W5005] Plain setTimeout() found in "${file}". Please use this.setTimeout() or adapter.setTimeout() instead.${clearNote}`,
        );
    }
...
// [E5004] Plain setInterval() found in "${file}". Please use this.setInterval() or adapter.setInterval() instead.
// [E5004] Plain setInterval() found in "${file}". Please use this.setInterval() or adapter.setInterval() instead. No clearInterval() detected.
// [W5004] Plain setInterval() found in "${file}". Please use this.setInterval() or adapter.setInterval() instead.
// [W5004] Plain setInterval() found in "${file}". Please use this.setInterval() or adapter.setInterval() instead. No clearInterval() detected.
// [W5005] Plain setTimeout() found in "${file}". Please use this.setTimeout() or adapter.setTimeout() instead.
// [W5005] Plain setTimeout() found in "${file}". Please use this.setTimeout() or adapter.setTimeout() instead. No clearTimeout() detected.
