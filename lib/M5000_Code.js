...
async function checkCode(context) {
    ...
    // Track files with bare setInterval() that do not clear the interval for W5004 check
    const setIntervalWithoutClearFiles = [];

    // Track files with bare setTimeout() for S5005 check (not this.setTimeout or adapter.setTimeout)
    const setTimeoutFiles = [];

    // Track files with bare setTimeout() that do not clear the timeout for W5005 check
    const setTimeoutWithoutClearFiles = [];
    ...
    for (const filePath of Object.keys(context)) {
        ...
        if (/(?<![.\w])setInterval\s*\(/.test(contentWithoutComments)) {
            ...
            if (!hasClearInterval) {
                setIntervalWithoutClearFiles.push(file);
            }
        }
        ...
        if (/(?<![.\w])setTimeout\s*\(/.test(contentWithoutComments)) {
            ...
            if (!hasClearTimeout) {
                setTimeoutWithoutClearFiles.push(file);
            }
        }
    ...
    if (setIntervalWithoutClearFiles.length > 0) {
        const fileList = setIntervalWithoutClearFiles.join(', ');
        context.warnings.push(
    ...
    if (setTimeoutWithoutClearFiles.length > 0) {
        const fileList = setTimeoutWithoutClearFiles.join(', ');
        context.warnings.push(
