...
const redundantPackages = require('./redundantPackages.js');
...
async function checkCode(context) {
    ...
    // Packages provided transitively by "@iobroker/testing" (>= 5.1.1). When such a version is
    // installed these packages (e.g. sinon, chai, mocha) do not need to be listed as (dev)dependencies,
    // so W5042 must not be raised for them.
    const testingProvidedPackages = new Set();
    const testingVersionRaw = context.packageJson?.devDependencies?.['@iobroker/testing'];
    if (testingVersionRaw) {
        try {
            const testingVersion = testingVersionRaw.replace(/[\^~]/, '').trim();
            if (compareVersions.compare(testingVersion, '5.1.1', '>=')) {
                for (const pkg of redundantPackages) {
                    testingProvidedPackages.add(pkg);
                }
            }
        } catch (e) {
    ...
    const excludedSourceRelPaths = new Set(['lib/tools.js', 'lib/tools.mjs', 'lib/tools.cjs']);
