...
function extractImports(content) {
    ...
    while ((match = requirePattern.exec(code)) !== null) {
        console.debug(`    require:     found ${match[1]}`);
        imports.add(match[1]);
    }
    ...
    const fromPattern = /\b(?:import)\b[\s\S]*?\bfrom\s+['"`]([^'"`\n]+)['"`]/g;
    while ((match = fromPattern.exec(code)) !== null) {
        console.debug(`    import from: found ${match[1]}`);
        imports.add(match[1]);
    }
    ...
    while ((match = sideEffectImportPattern.exec(code)) !== null) {
        console.debug(`    import ...:  found ${match[1]}`);
        imports.add(match[1]);
    }
    ...
    while ((match = dynamicImportPattern.exec(code)) !== null) {
        console.debug(`    dyn import:  found ${match[1]}`);
        imports.add(match[1]);
    }
