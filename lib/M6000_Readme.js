...
const levelTwoHeaderRegex = /^##\s+.+$/gm;
...
const sentryNoticeRegex = new RegExp(escapeRegExp(SENTRY_NOTICE_TEXT).replace(/\s+/g, '\\s+'));

function getSentryNoticeMatch(text) {
    return sentryNoticeRegex.exec(text);
}
    ...
    if (!data) {
        ...
        if (sentryPluginConfigured) {
            ...
            if (!sentryNoticeMatch) {
                ...
                const levelTwoHeaders = [...data.matchAll(levelTwoHeaderRegex)];
