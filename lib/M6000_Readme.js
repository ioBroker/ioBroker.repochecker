...
async function checkReadme(context) {
    ...
    if (!data) {
                ...
                for (const header of changelogHeaders) {
                    ...
                    if (!match) {
                        if (
                            ...
                            !header.includes('Older version') // includes 'Older versions'
                        ) {
        ...
        if (
            ...
            new RegExp(`install\\s+from\\s+custom\\s+url`, 'i').test(data) ||
            // Installation via Github Cat possible.
            new RegExp(`installation\\s+via\\s+github`, 'i').test(data) ||
