...
const SENTRY_NOTICE_REGEX_4 = new RegExp(escapeRegExp(SENTRY_NOTICE_TEXT_4).replace(/\s+/g, '\\s+'));

function getSentryNoticeMatch(text) {
    return SENTRY_NOTICE_REGEX_1.exec(text) || SENTRY_NOTICE_REGEX_2.exec(text) || SENTRY_NOTICE_REGEX_3.exec(text) || SENTRY_NOTICE_REGEX_4.exec(text);
}
...
// Minimum number of words the README license section must contain to be
// considered to hold the complete license text (instead of only a link).
const MIN_LICENSE_WORDS = 100;
...
async function checkReadme(context) {
    ...
    if (!data) {
        ...
        if (!licenseHeaderMatch) {
            ...
            // Check that the license section contains either the complete license
            // text (at least MIN_LICENSE_WORDS words) or a link to the LICENSE file.
            const licenseSection = findH2Section(data, 'license');
            const licenseWordCount = (licenseSection.trim().match(/\S+/g) || []).length;
            const hasLicenseFileLink = /\[[^\]]*\]\([^)]*LICENSE[^)]*\)/i.test(licenseSection);
            if (licenseWordCount < MIN_LICENSE_WORDS && !hasLicenseFileLink) {
                context.warnings.push(
                    '[W6034] "## License" section in README.md must contain either the complete license text or a link to the LICENSE file.',
                );
            } else {
                context.checks.push('README.md license section contains full license text or a link to LICENSE');
            }
        ...
        if (
            ...
            /(?:^|\n)[^\n]*iobroker\s+url\s+https:\/\/(\w+\.)?github\.com\//i.test(dataForInstallCheck) ||
            ...
            /install\s+from\s+(?:[^\n]*\s)?https?:\/\/github\.com\//i.test(dataForInstallCheck)
        ) {
            context.errors.push(
