...
const SENTRY_NOTICE_TEXT =
    '**This adapter uses Sentry libraries to automatically report exceptions and code errors to the developers.** For more details and for information on how to disable the error reporting, see <a href="https://github.com/ioBroker/plugin-sentry#plugin-sentry">Sentry-Plugin Documentation</a>!';
const LEVEL_TWO_HEADER_REGEX = /^##\s+.+$/gm;
...
const SENTRY_NOTICE_REGEX = new RegExp(escapeRegExp(SENTRY_NOTICE_TEXT).replace(/\s+/g, '\\s+'));

function getSentryNoticeMatch(text) {
    return SENTRY_NOTICE_REGEX.exec(text);
}
    ...
    if (!data) {
        ...
        if (sentryPluginConfigured) {
            ...
            if (!sentryNoticeMatch) {
                ...
                const levelTwoHeaders = [...data.matchAll(LEVEL_TWO_HEADER_REGEX)];
                if (levelTwoHeaders.length >= 3 && sentryNoticeMatch.index > levelTwoHeaders[2].index) {
                    context.warnings.push(
        ...
        if (pos === -1) {
            ...
            const textAfterLicense = data.substring(pos + '## License'.length);
        ...
        if (
            ...
            new RegExp(`(?:^|\\n)[^\\n]*npm\\s+(?:install|i)\\s+${ownerAdapterPattern}`, 'i').test(data) ||
