...
// A single German word may appear in otherwise-English text (e.g. a device name like "Gerät"),
// so require several matches to reduce false positives.
const MIN_GERMAN_WORDS_FOR_ERROR = 3;
...
/**
 * Search for German words from the blacklist in the given text.
 * Returns all matching words (deduplicated), or an empty array if none found.
 *
 * @param {string} text - The text to analyze
 * @returns {string[]} All distinct German words found
 */
function findGermanWords(text) {
    const found = new Set();
...
async function checkReadme(context) {
    ...
    if (!data) {
        ...
        const germanWords = findGermanWords(data);
        if (germanWords.length >= MIN_GERMAN_WORDS_FOR_ERROR) {
            context.errors.push(
                `[E6015] README.md appears to be written in German (detected words: "${germanWords.join('", "')}"). README.md must be written in English. Please translate.`,
            );
        } else {
            context.checks.push('No German language detected in README.md');
            if (!isEnglishText(data)) {
                context.warnings.push(
                    '[W6016] README.md does not appear to be written in English. README.md must be written in English. Please translate.',
                );
            } else {
...
// [E6015] README.md appears to be written in German (detected words: "..."). README.md must be written in English.
