 ...
 * @returns {Array<{attr: string, value: any, path: string, noTranslation: boolean, isSelectOptionLabel: boolean}>} - Collected attribute entries.
 */
    ...
    for (const { attr, value, path, noTranslation, isSelectOptionLabel } of translatables) {
        ...
        if (typeof value === 'object' && value !== null && !Array.isArray(value)) {
            if (isSelectOptionLabel || i18nMode === false) {
                ...
                // For i18n=false all inline i18n objects must be complete.
                for (const lang of requiredLanguages) {
