...
function checkI18nDirectory(context, i18nDir) {
    ...
    if (untranslatedSummary.length > 0) {
        context.warnings.push(
            `[W5606] i18n "${displayDir}" seems to contain untranslated entries identical to English (>=3 words). Affected languages: ${untranslatedSummary.map(entry => `${entry.lang} (${entry.count} key(s)${entry.examples.length ? `, ${entry.examples.length === 1 ? `example: ${entry.examples[0]}` : `examples: ${entry.examples.join(', ')}`}` : ''})`).join('; ')}.`,
        );
    }
}
