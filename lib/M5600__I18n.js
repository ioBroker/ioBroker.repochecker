...
function collectTranslatableValues(node, currentPath, state = { insideSelectOptions: false }) {
    ...
    for (const [key, value] of Object.entries(node)) {
        ...
        const childState = key === 'options' && isSelectComponent ? { ...state, insideSelectOptions: true } : state;
    ...
    for (const { attr, value, path, noTranslation, isSelectOptionLabel } of translatables) {
        ...
        if (typeof value === 'object' && value !== null && !Array.isArray(value)) {
            if (isSelectOptionLabel || i18nMode === false) {
                // Select-option labels as i18n objects must always be complete.
                // For i18n=false all inline i18n objects must also be complete.
                for (const lang of requiredLanguages) {
