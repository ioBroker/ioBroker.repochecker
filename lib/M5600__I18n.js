...
const I18N_EXCLUDED_DIRS = new Set([
    'node_modules', 
    '.git', 
    '.vscode', 
    '.dev-server',
...
// Strings that do not need a translation at all and therefore must not raise
// W5612/E5612. Compared case-insensitively against the attribute value.
const W5612_TRANSLATION_EXCLUSIONS = [
    'aabbccddeeff',
    'iobroker',
...
const MAX_W5606_EXAMPLES = 2;
const MIN_WORDS_FOR_W5606 = 5;
const MAX_56XX_ISSUES_PER_CODE = 10;
const ISSUE_CODE_56XX_REGEX = /^\[((?:E|W|S)56\d{2})\]/;

/**
 * Limit every E/W/S56xx issue code in the list to MAX_56XX_ISSUES_PER_CODE messages
 * plus one additional "maximum reached" message.
 *
 * @param {string[]} issues - Issue list (errors, warnings, or suggestions).
 * @returns {string[]} - Limited issue list.
 */
function limit56xxIssues(issues) {
    const limited = [];
    const perCodeCount = new Map();

    for (const issue of issues) {
        const match = issue.match(ISSUE_CODE_56XX_REGEX);
        if (!match) {
            limited.push(issue);
            continue;
        }
        ...
        const code = match[1];
        const count = perCodeCount.get(code) ?? 0;

        if (count < MAX_56XX_ISSUES_PER_CODE) {
            limited.push(issue);
            perCodeCount.set(code, count + 1);
            continue;
        }

        if (count === MAX_56XX_ISSUES_PER_CODE) {
            limited.push(
                `[${code}] Maximum number of ${code} issues (${MAX_56XX_ISSUES_PER_CODE}) has been reached. Additional ${code} issues exist.`,
            );
            perCodeCount.set(code, count + 1);
        }
    }
...
/**
 * Apply issue limits for all E/W/S56xx codes.
 *
 * @param {object} context - The repochecker context object.
 */
function apply56xxIssueLimits(context) {
    context.errors = limit56xxIssues(context.errors);
    context.warnings = limit56xxIssues(context.warnings);
    if (Array.isArray(context.suggestions)) {
        context.suggestions = limit56xxIssues(context.suggestions);
    }
}
...
/**
 * Count actual alphabetic words in a text.
 * Only sequences of alphabetic characters (including accented Latin letters) are counted,
 * so punctuation, brackets, and JSON-like syntax are ignored.
 * This function is intentionally applied to English text only, so Latin Unicode
 * coverage is sufficient.
 *
 * @param {string} text - Text to evaluate.
 * @returns {number} - Number of alphabetic words in the text.
 */
...
/**
 * Return true if the text contains a URL (http/https scheme or www. prefix).
 *
 * @param {string} text - Text to evaluate.
 * @returns {boolean} - Whether a URL was found.
 */
...
/**
 * Normalize language code to a 2-character representation for compact reporting.
 *
 * @param {string} lang - Language code (e.g. "de", "zh-cn").
 * @returns {string} - Two character language code.
 */
function toTwoCharLanguageCode(lang) {
...
/**
 * Format one W5606 language summary entry.
 *
 * @param {{lang: string, count: number, examples: string[]}} entry - Aggregated language data.
 * @returns {string} - Human-readable language summary.
 */
function formatW5606LanguageEntry(entry) {
    const details = [`${entry.count} ${entry.count === 1 ? 'key' : 'keys'}`];
    if (entry.examples.length === 1) {
        details.push(`example: ${entry.examples[0]}`);
    } else if (entry.examples.length > 1) {
        details.push(`examples: ${entry.examples.join(', ')}`);
    }

    return `${entry.lang} (${details.join(', ')})`;
}
...
function checkI18nDirectory(context, i18nDir) {
    ...
    for (const [lang, langTranslations] of translations) {
        ...
        // W5606: Language has untranslated entries identical to English (>3 words)
        const identical = [...enKeys].filter(
            k =>
                langKeys.has(k) &&
                typeof enTranslations[k] === 'string' &&
                typeof langTranslations[k] === 'string' &&
                !containsUrl(enTranslations[k]) &&
                countWords(enTranslations[k]) > MIN_WORDS_FOR_W5606 &&
                enTranslations[k] === langTranslations[k],
        );

        if (identical.length > 0) {
            untranslatedSummary.push({
                lang: toTwoCharLanguageCode(lang),
                count: identical.length,
                examples: identical.slice(0, MAX_W5606_EXAMPLES),
            });
        }
    }

    if (untranslatedSummary.length > 0) {
        context.warnings.push(
            `[W5606] i18n "${displayDir}" seems to contain untranslated entries identical to English (>${
                MIN_WORDS_FOR_W5606
            } words). Affected languages: ${untranslatedSummary.map(formatW5606LanguageEntry).join('; ')}.`,
        );
    }
}
 ...
 * @param {{insideSelectOptions: boolean}} [state] - recursion state tracking if traversal is currently inside options of a select* component.
 * @returns {Array<{
 *   attr: string,
 *   value: any,
 *   path: string,
 *   noTranslation: boolean,
 *   isSelectOptionLabel: boolean
 * }>} - Collected attribute entries. isSelectOptionLabel is true for label attributes found inside select* options.
 */
function collectTranslatableValues(node, currentPath, state = { insideSelectOptions: false }) {
    ...
    if (Array.isArray(node)) {
        node.forEach((item, i) => {
            results.push(...collectTranslatableValues(item, `${currentPath}[${i}]`, state));
        });
        return results;
    }
    ...
    const isSelectComponent = nodeType.startsWith('select');
    ...
    for (const [key, value] of Object.entries(node)) {
        if (TRANSLATABLE_ATTRS.includes(key) && value !== undefined && value !== null) {
            results.push({
                ...
                path: currentPath,
                noTranslation,
                isSelectOptionLabel: key === 'label' && state.insideSelectOptions,
            });
        }

        // Recurse into child objects and arrays
        const childState = key === 'options' && isSelectComponent ? { insideSelectOptions: true } : state;
        if (typeof value === 'object' && value !== null && !Array.isArray(value)) {
            results.push(...collectTranslatableValues(value, `${currentPath}/${key}`, childState));
        } else if (Array.isArray(value)) {
            value.forEach((item, i) => {
                if (typeof item === 'object' && item !== null) {
                    results.push(...collectTranslatableValues(item, `${currentPath}/${key}[${i}]`, childState));
                }
            });
        }
    }
...
function checkJsonConfigTranslations(context, configFile, jsonConfig, i18nDir, w5612State) {
    ...
    // W5619: i18n attribute missing - flag as it defaults to true which may be surprising
    if (i18nMode === undefined || i18nMode === null) {
        context.warnings.push(
            `[S5619] "${displayConfig}" does not specify the "i18n" attribute. It defaults to true, which may be unintentional. Please explicitly add "i18n": true, "i18n": false, or a custom path.`,
        );
        return;
    }
    ...
    for (const { attr, value, path, noTranslation, isSelectOptionLabel } of translatables) {
        ...
        // console.log(`type: ${typeof value} - ${JSON.stringify(value)}`);
        if (typeof value === 'object' && value !== null && !Array.isArray(value)) {
            if (isSelectOptionLabel || i18nMode === false) {
                // When i18n=false, or for select-option labels regardless of i18n mode,
                // all inline i18n objects must be complete.
                const missingRequiredLanguages = requiredLanguages.filter(
                    lang => !Object.prototype.hasOwnProperty.call(value, lang),
                );
                if (missingRequiredLanguages.length > 0) {
                    context.errors.push(
                        `[E5610] "${path}" attribute "${attr}" is missing required language(s) "${missingRequiredLanguages.join('", "')}" in i18n object.`,
                    );
                }

                const missingOptionalLanguages = allowedLanguages.filter(
                    lang => !requiredLanguages.includes(lang) && !Object.prototype.hasOwnProperty.call(value, lang),
                );
                if (missingOptionalLanguages.length > 0) {
                    context.warnings.push(
                        `[W5611] "${path}" attribute "${attr}" is missing language(s) "${missingOptionalLanguages.join('", "')}" in i18n object.`,
                    );
                }
                continue;
            }
            ...
            if (i18nMode === false) {
                ...
                if (!Object.prototype.hasOwnProperty.call(enTranslations, value)) {
                    if (W5612_TRANSLATION_EXCLUSIONS.includes(value.toLowerCase())) {
                        // Value is on the list of strings that never need a translation
                        continue;
                    }
...
// [W5606] Non-English translations identical to English (>3 words, no URL), grouped by affected language
...
// [E5618] "Custom path specified at i18n attribute (${customPath}) must end with '/i18n'
// [W5619] jsonConfig does not specify the "i18n" attribute (defaults to true)
