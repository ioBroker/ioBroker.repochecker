...
function formatW5606LanguageEntry(entry) {
    const details = [`${entry.count} key(s)`];
...
function checkI18nDirectory(context, i18nDir) {
    ...
    if (untranslatedSummary.length > 0) {
        context.warnings.push(
            `[W5606] i18n "${displayDir}" seems to contain untranslated entries identical to English (>=3 words). Affected languages: ${untranslatedSummary.map(formatW5606LanguageEntry).join('; ')}.`,
        );
    }
}
