...
function checkI18nDirectory(context, i18nDir) {
    ...
    if (longFormat.size > 0) {
        context.warnings.push(
            `[S5601] i18n directory "${displayDir}" uses long format ({lang}/translations.json). Consider migrating to short format ({lang}.json) using "npm run translate convert". See https://github.com/ioBroker/ioBroker.repochecker/issues/384`,
        );
    }
    ...
    for (const [lang, langTranslations] of translations) {
        ...
        if (identical.length > 5) {
            context.warnings.push(
                `[W5606] i18n "${displayDir}" language "${lang}" has ${identical.length} translation(s) identical to English. This may indicate untranslated content.`,
            );
        }
    }
}
 ...
 */
function checkJsonConfigTranslations(context, configFile, jsonConfig, i18nDir) {
...
function checkAllI18n(context) {
    ...
    for (const configFile of jsonConfigFiles) {
        ...
        const i18nDir = getI18nDirForJsonConfig(configFile, jsonConfig);
        checkJsonConfigTranslations(context, configFile, jsonConfig, i18nDir);
    }
}
