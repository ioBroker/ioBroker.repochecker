...
function checkI18nDirectory(context, i18nDir) {
    ...
    if (untranslatedSummary.length > 0) {
        context.warnings.push(
            `[W5606] i18n "${displayDir}" seems to contain untranslated entries identical to English (>=3 words). Affected languages: ${untranslatedSummary.map(entry => `${entry.lang} (${entry.count} key(s), e.g. ${entry.examples.join(', ')})`).join('; ')}.`,
        );
    }
}
