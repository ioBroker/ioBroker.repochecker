...
const filterByAdapter = {
    ...
    backitup: [
        '[E5050] process.exit() used in * (lib/restore.js)*',
        '[E5050] process.exit() used in * (src/lib/restore.ts)*',
        '[W0095] "prepublishOnly" script is not needed in most cases when using the standard ioBroker workflow test-and-release.*',
        '[W1144] Untranslated "common.news" entries detected*', // reduce disussions ...
        '[W8905] Repository contains package.json files in non-root directories*', // reduce disussions ...
        '[S5004] Plain setInterval() found in source files (src/lib/restore/mysql.ts*',
        '[S5051] Custom sleep/wait function(s) found in source files*',
    ],
    'schlueter-thermostat': [
        '[W1073] Potential sensitive data "apiKey" not listed at "protectedNative" in io-package.json',
        '[W1074] Potential sensitive data "apiKey" not listed at "encryptedNative" in io-package.json',
    ],
};

/**
 * Per-adapter severity remapping table (used by remapIssuesByAdapter).
 *
 * Outer key : adapter name without the "ioBroker." prefix, e.g. 'zigbee'.
 * Inner key : issue code, e.g. 'E3025'.
 * Value     : target severity – 'E' (error), 'W' (warning), 'S' (suggestion),
 *             or 'N' (drop entirely).
 */
const remapSeverityByAdapter = {
    zigbee: {
        E3025: 'W', // Node 26 test requirement not yet fulfilled; keep visible but reduce to warning
    },
};
...
const mapSeverity4New = {
    ...
    W0092: 'E', // [W0092] Script(s) "prepare" found in package.json. Usage of npm install lifecycle scripts should be avoided.
    ...
    W1144: 'E', // [W1144] Untranslated "common.news" entries detected (identical to English text): nl(3.4.11, 3.4.8).
    ...
    W4052: 'E', // [W4052] GitHub noreply email address "..." is not allowed. Please use a real email address.
    S4050: 'E', // [S4050] No valid email address found for copyright line in README.md
    S4051: 'E', // [S4051] No valid email address found for copyright line in LICENSE
    ...
    W3049: 'E', // [W3049] Workflow "test-and-release.yml": job "adapter-tests" step using "ioBroker/testing-action-adapter" specifies "integration-test-command". This command should not be changed; the default value should be used.
    S3051: 'E', // [S3051] "test-and-release.yml": "testing-action-check" in run #344 does not log the expected defaults for "npm run test:package". Please check the log
    S3053: 'E', // [S3053] "test-and-release.yml": "testing-action-adapter" job "adapter-tests (22.x, macos-latest)" in [run #333](https://github.com/mosandlt/ioBroker.bosch-smart-home-camera/actions/runs/28146512000) does not log the expected defaults for "npm run test:integration". Please check the [log](https://github.com/mosandlt/ioBroker.bosch-smart-home-camera/actions/runs/28146512000/job/83354759586).
    W3051: 'E', // [S3051] "test-and-release.yml": "testing-action-check" in run #344 does not log the expected defaults for "npm run test:package". Please check the log
    W3053: 'E', // [S3053] "test-and-release.yml": "testing-action-adapter" job "adapter-tests (22.x, macos-latest)" in [run #333](https://github.com/mosandlt/ioBroker.bosch-smart-home-camera/actions/runs/28146512000) does not log the expected defaults for "npm run test:integration". Please check the [log](https://github.com/mosandlt/ioBroker.bosch-smart-home-camera/actions/runs/28146512000/job/83354759586).
    ...
    W6034: 'E', // [W6034] "## License" section in README.md must contain either the complete license text or a link to the LICENSE file.
    W7006: 'E', // [W7006] LICENSE file must contain the complete license text.
    ...
    S5619: 'E', // [S5619] "${displayConfig}" does not specify the "i18n" attribute. It defaults to true, which may be unintentional. Please explicitly add "i18n": true, "i18n": false, or a custom path.`,
...
/**
 * Remaps the severity of issues for the current adapter according to remapByAdapter.
 *
 * If context.adapterName is not set, or no entry exists for that adapter
 * in remapByAdapter, the function returns immediately without changes.
 *
 * For every remapped issue an info message is logged via common.info().
 * If at least one issue was remapped a summary info is appended to
 * context.warnings.
 *
 * @param {object} context - The checker context
 * @returns {object} The (possibly modified) context
 */
function remapIssuesByAdapter(context) {
    const adapterName = context.adapterName;
    if (!adapterName) {
        return context;
    }

    const rules = remapSeverityByAdapter[adapterName];
    ...
    /**
     * Scan one issue array in-place: remove matching entries and re-insert
     * them with the new severity.
     *
     * @param {string[]} arr - context.errors or context.warnings
     */
    function processArray(arr) {
        ...
        for (let i = arr.length - 1; i >= 0; i--) {
            const issue = arr[i];
            const match = issue.match(/^\[([EWSI])(\d+)\]/);
            if (!match) {
                continue;
            }
            const issueKey = match[1] + match[2]; // e.g. "E3025"
            if (!Object.prototype.hasOwnProperty.call(rules, issueKey)) {
                continue;
            }
            const targetSeverity = rules[issueKey];
            common.info(
                `[postprocessing] remapIssuesByAdapter: remapping issue for adapter "${adapterName}" to ${targetSeverity}: ${issue}`,
            );
            arr.splice(i, 1);
            toReroute.push({ targetSeverity, issue });
            remappedCount++;
        }

        for (const { targetSeverity, issue } of toReroute) {
            if (targetSeverity === 'N') {
                continue; // drop entirely
            }
            // Replace the leading [X####] prefix with the new severity letter
            const newIssue = issue.replace(/^\[([EWSI])(\d+)\]/, `[${targetSeverity}$2]`);
            if (targetSeverity === 'E') {
                context.errors.push(newIssue);
            } else {
                context.warnings.push(newIssue);
            }
        }
    }
    ...
    if (remappedCount > 0) {
        common.info(
            `[postprocessing] remapIssuesByAdapter: ${remappedCount} issue(s) remapped for adapter "${adapterName}".`,
        );
        context.warnings.push(`[S0000] Info: ${remappedCount} issue(s) have adjusted severity for this adapter.`);
    }
