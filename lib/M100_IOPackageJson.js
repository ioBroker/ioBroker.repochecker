...
async function validateIOPackageSchema(context) {
    ...
    try {
        ...
        if (!schemaData) {
            context.error.push('[E207] Could not download ${schemaUrl}schema for validation of io-package.json');
            return context;
        }
        ...
        if (valid) {
            ...
            if (validate.errors) {
                for (const error of validate.errors) {
                    ...
                    //console.log(`Schema validation error block:\n ${JSON.stringify(error)}`);
                    ...
                    if (error.keyword === 'required') {
                        ...
                        errorMsg = `Unknown property "${error.params.additionalProperty}" in ${path || 'io-package.json'}`;
                    ...
                    context.warnings.push(`[W205] io-package.json schema validation failed: ${errorMsg}`);
                }
            } else {
                context.warnings.push('[W205] io-package.json does not validate against schema');
            }
        }
    } catch (error) {
        common.debug(`Schema validation error: ${error.message}`);
        context.warnings.push(`[W208] Schema validation failed: ${error.message}`);
    }
        ...
        if (!context.cfg.allowedModes[context.ioPackageJson.common.mode]) {
            ...
            if (
                ...
                if (context.ioPackageJson.common.allowInit === true) {
                    context.warnings.push(
                        '[W204] allowInit is only used with scheduled adapters and should be removed',
                    );
                } else if (context.ioPackageJson.common.allowInit === false) {
                    context.warnings.push(
                        '[S204] allowInit is only used with scheduled adapters and should be removed',
                    );
                }
            }
        }
...
// [204] allowInit is only used with scheduled adapters and should be removed
// [205] Schema validation: ${errorMsg}
// [207] Could not download io-package.json schema for validation
// [208] Schema validation failed: ${error.message}
