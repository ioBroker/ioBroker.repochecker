...
async function checkIOPackageJson(context) {
    ...
    if (!context.ioPackageJson.native) {
        ...
        const suspiciousKeys = nativeKeys.filter(key => regex.test(key.toLowerCase()));
        common.debug(`Suspicious key test: native: ${nativeKeys} - suspiciousKeys: ${suspiciousKeys}`);
        ...
        for (const key of context.ioPackageJson.encryptedNative || []) {
            if (!context.ioPackageJson.protectedNative.includes(key)) {
                context.warnings.push(
                    `[W193] Encrypted native "${key}" not listed at "protectedNative" in io-package.json`,
                );
            }
        }

        for (const key of context.ioPackageJson.protectedNative || []) {
            if (!context.ioPackageJson.encryptedNative.includes(key)) {
                context.warnings.push(
                    `[W194] Protected native "${key}" not listed at "encryptedNative" in io-package.json`,
                );
            }
        }
        ...
        for (const key of [
            ...
            if (!context.ioPackageJson.native[key]){
                context.warnings.push(
                    `[W195] Protected or encrypted element "${key}" not listed as "native" in io-package.json`,
                );
            }
        }
    }
    ...
    if (context.ioPackageJson.common.extIcon) {
        try {
            ...
            common.debug(`logo ${context.ioPackageJson.common.icon} width: ${image.width}, height: ${image.height}`);
            if (image.width !== image.height) {
                context.errors.push('[E140] width and height of logo (specified at "common.extIcon") are not equal');
            } else {
