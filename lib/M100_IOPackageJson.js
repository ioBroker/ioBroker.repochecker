...
async function checkIOPackageJson(context) {
    ...
    if (!context.ioPackageJson.common) {
        ...
        // Todo-oliverio 'resolve' is not defined.
        ...
        if (!context.ioPackageJson.common.desc) {
            ...
            if (typeof context.ioPackageJson.common.desc !== 'object') {
                ...
                if (missingLang.length) {
                    context.errors.push(`[E133] Missing mandatory translation into ${missingLang.join()} of "common.desc" in io-package.json.`);
                }
        ...
        if (!context.ioPackageJson.common.compact && !context.ioPackageJson.common.onlyWWW) {
            if (context.ioPackageJson.common.compact === undefined) {
                context.warnings.push('[W113] Please test whether adapter can support compact mode, if not set "common.compact:false" at io-package.json');
            } else {
        ...
        if (!context.ioPackageJson.common.news) {
            ...
            if (missingLang.length) {
                missingLang = [... new Set(missingLang)]; // make unique
                context.errors.push(`[E145] Missing mandatory translation into ${missingLang.join()} of some "common.news" in io-package.json.`);
            }
        ...
        if ((context.ioPackageJson.common.mode !== 'none') && (context.ioPackageJson.common.mode !== 'extension')) {
            if (!context.packageJson.main) {
                ...
                if (!context.packageJson.main.endsWith('.js') && !context.packageJson.main.endsWith('.ts')) {
                    !context.ioPackageJson.common.onlyWWW && context.errors.push(`[E163] common.mode "${context.ioPackageJson.common.mode}" requires JavaScript or Typescript file for "main" in package.json`);
                } else {
        ...
        if (!context.ioPackageJson.common.onlyWWW && context.packageJson.main) {
            try {
                ...
                if (!context.ioPackageJson.common.nogit) {
                    ...
                    context.warnings.push(`[W138] Main file not found under URL: ${context.githubUrl}/${context.packageJson.main} and not located at build/ directory too.`);
                }
            }
        }
    }
