...
async function validateIOPackageSchema(context) {
    ...
    try {
        ...
        if (!schemaData) {
            context.warnings.push('[W207] Could not download ${schemaUrl} for validation of io-package.json');
            return context;
        }
        ...
        if (valid) {
            ...
            if (validate.errors) {
                for (const error of validate.errors) {
                    ...
                    context.warnings.push(`[W205] io-package.json schema validation error: ${errorMsg}`);
                }
            } else {
        ...
        context.warnings.push(`[W208] io-package.json schema validation failed: ${error.message}`);
    }
