...
async function checkCode(context) {
    ...
    if (context.filesList.includes('/lib/tools.js')) {
        ...
        if (!isUsed) {
            context.warnings.push(
                '[W532] "lib/tools.js" found in repo but not used anywhere. Consider removing this outdated file.',
            );
        }
    }
                    ...
                    for (const pattern of patterns) {
                        if (pattern.test(line)) {
                            ...
                            if (!functionDefPattern.test(line)) {
                                context.warnings.push(
                                    `[W533] "${fileName}:${lineNumber}" uses deprecated method "${method}()". This method will be removed in js-controller 6. Please use setObject() or deleteObject() instead.`,
                                );
                            }
...
// [533] "${fileName}:${lineNumber}" uses deprecated method "${method}()". This method will be removed in js-controller 6. Please use setObject() or deleteObject() instead.
