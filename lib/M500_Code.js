...
function getAllFiles(context, dirPath, root, filesList) {
    const skipDirectories = ['.dev-server', 'node_modules', '.git', '.vscode', 'admin', 'admin-src'];
...
// Helper function to discover all files without reading them (for executable file discovery)
function getAllFilesForDiscovery(dirPath, root, filesList) {
    ...
    files.forEach(file => {
        if (fs.statSync(path.join(dirPath, file)).isDirectory()) {
            if (!skipDirectories.includes(file)) {
                filesList = getAllFilesForDiscovery(path.join(dirPath, file), [root, file].join('/'), filesList);
            }
        } else {
            filesList.push([root, file].join('/').slice(1));
        }
...
async function checkCode(context) {
    ...
    if (common.isLocal()) {
        ...
        tempFilesList = getAllFilesForDiscovery(process.cwd(), '', []);
    } else {
    ...
    if (context.filesList.includes('/lib/tools.js')) {
        ...
        if (!isUsed) {
            context.warnings.push(
                '[W531] "lib/tools.js" found in repo but not used anywhere. Consider removing this outdated file.',
            );
        }
    }
...
// [531] "lib/tools.js" found in repo but not used anywhere. Consider removing this outdated file.
