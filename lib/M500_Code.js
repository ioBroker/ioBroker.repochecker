...
async function checkCode(context) {
    ...
    if (common.isLocal()) {
        ...
        common.log(`${context.branch}.zip ${data.length} bytes`);
    ...
    if (
        ...
        common.log('REACT detected at package devDependencies');
        usesReact = true;
    }
    if (
        ...
        common.log('REACT detected at package dependencies');
        usesReact = true;
    }
    if (context['/src-admin/package.json']) {
        //common.log('"src-admin/package.json" exists');
        ...
        if (packageJson.devDependencies && packageJson.devDependencies['@iobroker/adapter-react-v5']) {
            common.log('REACT detected at src-admin/package devDependencies');
            usesReact = true;
        }
        if (packageJson.dependencies && packageJson.dependencies['@iobroker/adapter-react-v5']) {
            common.log('REACT detected at src-admin/package dependencies');
            usesReact = true;
        }
    }
    if (context['/src-widgets/package.json']) {
        //common.log('"src-widgets/package.json" exists');
        ...
        if (
            ...
            common.log('REACT detected at src-widgets/package devDependencies');
            usesReact = true;
        }
        if (
            ...
            common.log('REACT detected at src-widgets/package dependencies');
            usesReact = true;
        }
    }
    if (context['/src/package.json']) {
        //common.log('"src/package.json" exists');
        const packageJson = JSON.parse(context['/src/package.json']);
        if (packageJson.devDependencies && packageJson.devDependencies['@iobroker/adapter-react-v5']) {
            // common.log('REACT detected');
            usesReact = true;
        }
        if (packageJson.dependencies && packageJson.dependencies['@iobroker/adapter-react-v5']) {
            // common.log('REACT detected');
    ...
    if (context.ioPackageJson.common.adminUI && context.ioPackageJson.common.adminUI.config === 'json') {
        ...
        if (jsonConfig) {
            ...
            M500__jsonConfig.checkJsonConfig(context, `admin/jsonConfig.json${context['/admin/jsonConfig.json'] ? '' : '5'}`, jsonConfig);
