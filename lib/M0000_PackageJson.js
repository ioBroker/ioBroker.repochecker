...
function applyConditionalRequirements(dependencyTable, packageJsonDeps) {
    ...
    for (const sourceDep in dependencyTable) {
        ...
        for (const condition of config.conditionalRequirements) {
            const { ifVersion, targetDependency, required, recommended } = condition;
...
async function checkNpmDependencyUpdateSuggestions(context) {
    ...
    for (const checkItem of deduplicatedChecks.values()) {
        ...
        try {
            ...
            context.warnings.push(
                `[${updateRule.level}0082] Package "${packageName}" (${sourceType}) can be updated from ${packageVersion} to ${latestVersion}. The newer version has been published for ${packageAgeDays} days. Please evaluate updating package.json.`,
            );
        } catch (e) {
            common.debug(`Could not check npm update age for "${packageName}": ${e.message || e}`);
        }
    }
...
async function checkPackageJson(context) {
        ...
        if (!context.packageJson.engines) {
            ...
            if (!context.packageJson.engines.node) {
                ...
                if (!match) {
                    ...
                    minimumNodeJsSupported = match.groups.vers;
    ...
    if (eslintVersion && semver.lt(eslintVersion, '9.0.0')) {
        context.warnings.push(
            `[W0072] "eslint":"${eslintVersionRaw}" is below 9.0.0. Please migrate to "@iobroker/eslint-config".`,
        );
    ...
    if (hasEslintConfig) {
        ...
        if (foundObsoleteDeps.length > 0) {
            context.warnings.push(
                `[W0078] Remove obsolete eslint/prettier devDependencies when using "@iobroker/eslint-config": ${foundObsoleteDeps.join(', ')}.`,
            );
        }
    }
...
async function checkEslintFileChecks(context) {
    ...
    const eslintVersion = eslintVersionRaw ? semver.coerce(eslintVersionRaw) : null;
    const hasEslintV9OrHigher = !!(eslintVersion && semver.gte(eslintVersion, '9.0.0'));
    const obsoleteEslintFiles = ['/.eslintignore', '/.eslintrc.json', '/.prettierignore', '/.prettierrc.js'];
    ...
    if (hasEslintConfig) {
        ...
        if (foundObsoleteFiles.length > 0) {
            context.warnings.push(
                `[W0079] Remove obsolete eslint/prettier config files when using "@iobroker/eslint-config": ${foundObsoleteFiles.map(file => file.slice(1)).join(', ')}.`,
            );
        }
...
// [W0082] Package "${packageName}" (${sourceType}) can be updated from ${packageVersion} to ${latestVersion}. ...
