            ...
            if (!context.packageJson.engines.node) {
                ...
                if (!minimumNodeJsSupported) {
                    ...
                    if (!String(nodeVal).trim().match(/^>=?/)) {
                        context.warnings.push(
                            `[W0028] Minimum node.js version ${recommendedNodeVersion} recommended. Please change "{'engines' : { 'node' >= '${minimumNodeJsSupported}' } }" to "{'engines' : { 'node' >= '${recommendedNodeVersion}' } }" at package.json.`,
                        );
                    } else {
                        if (!compareVersions.compare(minimumNodeJsSupported, requiredNodeVersion, '>=')) {
                            context.errors.push(
                                `[E0029] Node.js ${requiredNodeVersion} is required as minimum, node.js ${recommendedNodeVersion} is recommended. Please change "{'engines' : { 'node' >= '${minimumNodeJsSupported}' } }" to "{'engines' : { 'node' >= '${requiredNodeVersion}' } }" at package.json.`,
                            );
                        } else if (!compareVersions.compare(minimumNodeJsSupported, recommendedNodeVersion, '>=')) {
                            context.warnings.push(
                                `[W0028] Minimum node.js version ${recommendedNodeVersion} recommended. Please adapt "{'engines' : { 'node' >= '${minimumNodeJsSupported}' } }" to "{'engines' : { 'node' >= '${recommendedNodeVersion}' } }" at package.json.`,
                            );
                        } else {
                            context.checks.push(
