    ...
    if (hasEslintConfig) {
        if (!fileExistsInList(context, '/eslint.config.mjs')) {
            ...
            try {
                ...
                context.errors.push(`[E0075] Could not read "eslint.config.mjs": ${e.message || e}`);
            }
        }
...
async function checkDependencyNodeRequirements(context) {
    ...
    const hasPackageLock = context.filesList && context.filesList.includes('/package-lock.json');
