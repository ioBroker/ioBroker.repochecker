...
async function checkPackageJson(context) {
    ...
    for (const dependency in dependenciesPackageJson) {
        ...
        if (exists) {
            ...
            if (!compareVersions.compare(dependencyVersion, requiredVersion, '>=')) {
                context.errors.push(
                    `[E0033] ${dependency} ${dependencyVersion} specified. ${requiredVersion} is required as minimum, ${recommendedVersion} (or newer) is current. Please update dependencies at package.json`,
                );
            } else if (!compareVersions.compare(dependencyVersion, recommendedVersion, '>=')) {
                context.warnings.push(
                    `[W0034] ${dependency} ${dependencyVersion} specified. ${recommendedVersion} (or newer) is current. Please update dependencies at package.json`,
                );
            } else {
    ...
    for (const dependency in devDependenciesPackageJson) {
        ...
        if (exists) {
            ...
            if (!compareVersions.compare(dependencyVersion, requiredVersion, '>=')) {
                context.errors.push(
                    `[E0036] ${dependency} ${dependencyVersion} specified. ${requiredVersion} is required as minimum,  ${recommendedVersion} (or newer) is current. Please update devDependencies at package.json`,
                );
            } else if (!compareVersions.compare(dependencyVersion, recommendedVersion, '>=')) {
                context.warnings.push(
                    `[W0037] ${dependency} ${dependencyVersion} specified. ${recommendedVersion} (or newer) is current. Please update devDependencies at package.json`,
                );
            } else {
    ...
    // check for outdated eslint version
    if (context.packageJson.devDependencies && context.packageJson.devDependencies.eslint) {
        let eslintVersion = context.packageJson.devDependencies.eslint;
        eslintVersion = eslintVersion.replace(/[\^~]/, '').trim();

        if (compareVersions.compare(eslintVersion, '9.0.0', '<')) {
            context.warnings.push(
                `[W0063] eslint ${eslintVersion} is outdated. eslint 9.0.0 or newer is recommended. Please update devDependencies at package.json`,
            );
        } else {
            context.checks.push(`eslint ${eslintVersion} is up-to-date`);
        }
    }
...
// [0063] eslint ${eslintVersion} is outdated. eslint 9.0.0 or newer is recommended. Please update devDependencies at package.json
