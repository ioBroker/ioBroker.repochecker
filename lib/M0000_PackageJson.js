...
const minimumNodeVersion = '18'; // This is the minimum node version that must be required by all adapters at engines clause
...
async function checkPackageJson(context) {
    ...
    if (context.packageJson.scripts && typeof context.packageJson.scripts === 'object') {
        const warningInstallScripts = ['preinstall'].filter(scriptName =>
            Object.prototype.hasOwnProperty.call(context.packageJson.scripts, scriptName),
        );
        if (warningInstallScripts.length) {
            context.warnings.push(
                `[W0092] Script(s) ${warningInstallScripts.map(scriptName => `"${scriptName}"`).join(', ')} found in package.json. Usage of npm install lifecycle scripts should be avoided.`,
            );
        }

        const forbiddenInstallScripts = ['install', 'postinstall'].filter(scriptName =>
            Object.prototype.hasOwnProperty.call(context.packageJson.scripts, scriptName),
        );
        if (forbiddenInstallScripts.length) {
            context.errors.push(
                `[E0093] Script(s) ${forbiddenInstallScripts.map(scriptName => `"${scriptName}"`).join(', ')} found in package.json. Remove npm install lifecycle scripts.`,
            );
        }

        const forbiddenScripts = ['preprepare', 'prepare', 'postprepare'];
