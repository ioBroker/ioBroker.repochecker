        ...
        if (!typesNodeVersion) {
            ...
            try {
                if (!semver.validRange(typesNodeVersion)) {
                    context.error.push(
                        `[E0068] Invalid semver format ${typesNodeVersion} for devDependency "@types/node" at package.json.`,
                    );
                } else {
                    // Check if @types/node allows a higher major version than the minimum
                    if (semver.satisfies(`>=${minMajor + 1}.0.0`, typesNodeVersion)) {
                        context.warnings.push(
                            `[W0066] "@types/node":"${typesNodeVersion}" at package.json allows node.js major version ${minMajor + 1} or higher. Consider using "@types/node":"^${minMajor}.0.0".`,
                        );
