...
async function checkPackageJson(context) {
    ...
    // check @types/node devDependency vs engines.node minimum version
        ...
        if (!typesNodeVersion) {
            ...
            try {
                if (!semver.validRange(typesNodeVersion)) {
                    common.debug(`Could not parse @types/node version range: ${typesNodeVersion}`);
                } else {
                    // Check if @types/node allows a higher major version than the minimum
                    if (semver.satisfies(`${minMajor + 1}.0.0`, typesNodeVersion)) {
                        context.warnings.push(
                            `[W0066] "@types/node":"${typesNodeVersion}" at package.json allows node.js major version ${minMajor + 1} or higher. Consider using "@types/node":"^${minMajor}.0.0" or ">=${minMajor} <${minMajor + 1}" to match the minimum node.js version ${minimumNodeJsSupported} specified in engines clause.`,
                        );
                    }
                    // Check if @types/node does not cover the minimum node version
                    if (!semver.satisfies(`${minMajor}.0.0`, typesNodeVersion)) {
                        context.warnings.push(
                    ...
                    if (
                        !semver.satisfies(`${minMajor + 1}.0.0`, typesNodeVersion) &&
                        semver.satisfies(`${minMajor}.0.0`, typesNodeVersion)
                    ) {
                        context.checks.push(
