        ...
        */
        ...
        for (const entry of findIgnoredTrackedEntries(trackedEntries, rules)) {
            if (entry.path === '/build' && entry.isDirectory) {
                context.errors.push(
                    '[E9007] build directory found in repository and is excluded by .gitignore. Remove the "/build" ignore rule if /build should stay tracked, or remove /build from git if it should only be generated locally.',
                );
                continue;
            }

            context.warnings.push(
                `[W9008] ${entry.isDirectory ? 'directory' : 'file'} ${entry.path.replace(/^\//, '')} found in repository but ignored by .gitignore. Check whether it should be removed from git as generated content, or whether the ignore rule should be removed because the path is intentionally tracked.`,
            );
        }
    }
