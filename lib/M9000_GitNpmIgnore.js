...
function getTrackedRepoEntries(filesList) {
    ...
    for (const file of filesList) {
        ...
        for (let i = 1; i < parts.length; i++) {
            entries.set(`/${parts.slice(0, i).join('/')}`, { path: `/${parts.slice(0, i).join('/')}`, isDirectory: true });
        }
    }
    ...
    return [...entries.values()].sort((a, b) => {
        ...
        return depthA - depthB || Number(b.isDirectory) - Number(a.isDirectory) || a.path.localeCompare(b.path);
    });
}
...
async function checkGitIgnore(context) {
    ...
    if (!context.filesList.includes('/.gitignore')) {
        ...
        const trackedEntries = getTrackedRepoEntries(context.filesList);
        ...
        */
        ...
        checkFiles.forEach(file => {
            if (trackedPaths.has(`/${file}`) && !isPathIgnoredByRules(`/${file}`, true, rules)) {
        ...
        for (const entry of findIgnoredTrackedEntries(trackedEntries, rules)) {
            if (entry.path === '/build' && entry.isDirectory) {
                context.errors.push(
                    '[E9007] build directory found in repository but is excluded by .gitignore. Please remove "/build" from .gitignore.',
                );
                continue;
            }

            context.warnings.push(
                `[W9008] ${entry.isDirectory ? 'directory' : 'file'} ${entry.path.replace(/^\//, '')} found in repository but ignored by .gitignore. Please check whether it is generated and should be removed from git.`,
            );
        }
    }
