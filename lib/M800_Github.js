...
async function checkGithubRepo(context) {
    ...
    for (const issue of issues) {
        ...
        if (issuesToWatchAsError.includes(issue.title)) {
            context.errors.push(
                `[E804] Please process ${type} [[#${issue.number}] "${issue.title}"](${context.githubUrlOriginal}/issues/${issue.number})`,
            );
        } else if (issuesToWatchAsWarning.includes(issue.title)) {
            context.warnings.push(
                `[W804] Please process ${type} [[#${issue.number}] "${issue.title}"](${context.githubUrlOriginal}/issues/${issue.number})`,
            );
