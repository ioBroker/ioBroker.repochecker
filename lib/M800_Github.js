...
async function getCommitInfos(context) {
    ...
    try {
        ...
        common.log(`[INTERNAL] error downloading commit info from ${url} - ${e}`);
        throw e;
    }
...
async function checkGithubRepo(context) {
    common.log('[E8xx] checkGithubRepo');
    ...
    for (const issue of issues) {
        ...
        if (issuesToWatch.includes(issue.title)) {
            context.errors.push(`[E804] Please process issue [[#${issue.number}] "${issue.title}"](${context.githubUrlOriginal}/issues/${issue.number})`);
        }
    }
