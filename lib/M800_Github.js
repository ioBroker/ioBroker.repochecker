...
const issuesToWatchAsWarning = ['Migration to ESLint 9 and @iobroker/eslint-config'];
    ...
    for (const issue of issues) {
        ...
        if (issuesToWatchAsError.includes(issue.title)) {
            ...
            context.warnings.push(
                `[W804] Please process issue [[#${issue.number}] "${issue.title}"](${context.githubUrlOriginal}/issues/${issue.number})`,
            );
