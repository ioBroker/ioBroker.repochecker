...
function matchesCommonType(value, commonType) {
    switch (commonType) {
        ...
        case 'string':
        case 'file':
        case 'json':       return typeof value === 'string';
        case 'multistate': return ['string', 'number', 'boolean'].includes(typeof value);
...
function validateStateCommon(stateId, common, collector, roleRuleMaps) {
    ...
    for (const [schemaKey, schema] of Object.entries(STATE_SCHEMA)) {
        ...
        if (schema.required && !hasValue) {
            collector.error('E1003',
                `Object "${stateId}": common is missing required key "${schemaKey}".`);
            continue;
        }
...
function validateHierarchyObjectOrder(objectsById, idsToCheck, collector) {
    for (const objectId of idsToCheck) {
        ...
        if (hasHierarchyMember && chain.some(e => !HIERARCHY_TYPES.has(e.type))) {
            collector.error('E2001',
                `Object "${objectId}": hierarchy contains non-hierarchy object types.`);
        }

        if (chain.filter(e => e.type === 'device').length > 1) {
            collector.error('E2002',
                `Object "${objectId}": hierarchy contains more than one device.`);
        }

        const firstChannelIdx = chain.findIndex(e => e.type === 'channel');
        if (firstChannelIdx !== -1 &&
            chain.slice(firstChannelIdx + 1).some(e => e.type === 'device')) {
            collector.error('E2003',
                `Object "${objectId}": hierarchy contains a device after a channel.`);
        }

        for (let i = 0; i < chain.length - 1; i++) {
            if (chain[i].type === 'state') {
                collector.error('E2004',
                    `Object "${objectId}": state object "${chain[i].id}" has children.`);
                break;
            }
        }
    }
...
function validateObjectDump(objects, adapter, collector) {
    ...
    for (const [rootKey, rootObj] of entries) {
        if (!isPlainObject(rootObj) || rootObj._id !== rootKey) {
            collector.error('E3002',
                `Root object "${rootKey}": invalid — expected a plain object with _id === "${rootKey}".`);
        }
    }

    if (collector.errors.length > 0) {
        collector.error('E3003',
            'Input is not a valid ioBroker object dump — processing aborted.');
        return [];
    }
    ...
    for (const [objectId, objectData] of entries) {
        ...
        for (const [requiredKey, expectedType] of Object.entries(ROOT_OBJECT_SCHEMA)) {
            if (!Object.prototype.hasOwnProperty.call(objectData, requiredKey)) {
                collector.error('E3004',
                    `Object "${objectId}": missing required key "${requiredKey}".`);
                continue;
            }
            const actualType = getValueType(objectData[requiredKey]);
            if (actualType !== expectedType) {
                collector.error('E3005',
                    `Object "${objectId}": key "${requiredKey}" must be type ${expectedType}, got ${actualType}.`);
            }
        }

        // --- No extra root keys ---
        const unsupportedKeys = Object.keys(objectData)
            .filter(k => !Object.prototype.hasOwnProperty.call(ROOT_OBJECT_SCHEMA, k));
        if (unsupportedKeys.length > 0) {
            collector.error('E3006',
                `Object "${objectId}": contains unsupported root keys: ${unsupportedKeys.join(', ')}.`);
        }

        // --- Known object type ---
        if (!ALLOWED_OBJECT_TYPES.has(objectData.type)) {
            collector.error('E3007',
                `Object "${objectId}": unknown type "${objectData.type}".`);
        }
    ...
    for (const objectId of relevantIds) {
        ...
        for (let i = 3; i < levels.length; i++) {
            ...
            if (!Object.prototype.hasOwnProperty.call(objects, prefix)) {
                collector.error('E3009',
                    `Object "${objectId}": missing intermediate object "${prefix}".`);
            }
        }
    }
    ...
    for (const objectId of relevantIds) {
        ...
        if (!isPlainObject(common) ||
            !Object.prototype.hasOwnProperty.call(common, 'type')) {
            collector.error('E3010',
                `Object "${objectId}": state must define common.type.`);
            continue;
        }

        if (!VALID_STATE_TYPES.has(common.type)) {
            collector.error('E3011',
                `Object "${objectId}": invalid common.type "${common.type}".`);
        }
