...
// ---------------------------------------------------------------------------
// Role rules used for validation. Uses the schema rules from config_StateRoles.js
// as-is; button roles keep their read/write constraints (read:false, write:true)
// so that misconfigured buttons are reported.
// ---------------------------------------------------------------------------
const ROLE_RULES = STATE_ROLE_RULES;
...
const I18N_REQUIRED_KEYS = ['en', 'de'];
const I18N_ALLOWED_KEYS = new Set(['en', 'de', 'ru', 'pt', 'nl', 'fr', 'it', 'es', 'pl', 'uk', 'zh-cn']);
const I18N_RECOMMENDED_KEYS = ['ru', 'pt', 'nl', 'fr', 'it', 'es', 'pl', 'uk', 'zh-cn'];

/**
 * Validates a value that must be either a plain string or an i18n object.
 * Returns an array of error/warning strings (empty = valid).
 *
 * @param {*}      value     - The value to validate.
 * @param {string} fieldPath - Human-readable path for error messages (e.g. "common.desc").
 * @returns {{ errors: string[], warnings: string[] }}
 */
function validateI18n(value, fieldPath) {
    const errors = [];
    const warnings = [];

    if (typeof value === 'string') return { errors, warnings };

    if (!isPlainObject(value)) {
        errors.push(`"${fieldPath}" must be a string or an i18n object.`);
        return { errors, warnings };
    }

    // Must be an object — validate as i18n
    const keys = Object.keys(value);

    for (const key of keys) {
        if (!I18N_ALLOWED_KEYS.has(key)) {
            errors.push(`"${fieldPath}" i18n object contains unknown key "${key}".`);
        }
        if (typeof value[key] !== 'string') {
            errors.push(`"${fieldPath}" i18n object key "${key}" must be a string.`);
        }
    }

    for (const required of I18N_REQUIRED_KEYS) {
        if (!Object.prototype.hasOwnProperty.call(value, required)) {
            errors.push(`"${fieldPath}" i18n object is missing required key "${required}".`);
        }
    }

    const missingRecommended = I18N_RECOMMENDED_KEYS.filter(k => !Object.prototype.hasOwnProperty.call(value, k));
    if (missingRecommended.length > 0) {
        warnings.push(`"${fieldPath}" i18n object is missing recommended keys: ${missingRecommended.join(', ')}.`);
    }
...
/**
 * Validates a JSON-serialized value and returns the parsed result when valid.
 *
 * @param {*} value
 * @returns {object}
 */
...
/**
 * Validates the common.enums array.
 */
function validateEnums(objectId, enums, collector) {
    if (!Array.isArray(enums)) {
        collector.error('E4001', `Object "${objectId}": common.enums must be an array.`);
        return;
    }
    for (let i = 0; i < enums.length; i++) {
        const entry = enums[i];
        const prefix = `Object "${objectId}": common.enums[${i}]`;
        if (!isPlainObject(entry)) {
            collector.error('E4002', `${prefix} must be an object.`);
            continue;
        }
        if (typeof entry._id !== 'string') {
            collector.error('E4003', `${prefix} must have a string "_id".`);
        }
        if (entry.type !== 'enum') {
            collector.error('E4004', `${prefix} must have "type" = "enum".`);
        }
        if (!isPlainObject(entry.common)) {
            collector.error('E4005', `${prefix} must have an object "common".`);
            continue;
        }
        if (entry.common.name === undefined) {
            collector.error('E4006', `${prefix}.common must have a "name".`);
        }
        const allowedCommonKeys = new Set(['name', 'smartName', 'icon', 'color', 'desc']);
        const unknownKeys = Object.keys(entry.common).filter(k => !allowedCommonKeys.has(k));
        if (unknownKeys.length > 0) {
            collector.warning('W4001', `${prefix}.common contains unexpected keys: ${unknownKeys.join(', ')}.`);
        }
    }
}
...
function matchesCommonType(value, commonType) {
    switch (commonType) {
        case 'mixed':
        case 'json':
            return parseSerializedValue(value).ok;
        case 'array': {
            const { ok, parsed } = parseSerializedValue(value);
        ...
        case 'file': {
            const { ok, parsed } = parseSerializedValue(value);
            return ok && typeof parsed === 'string';
        }
        case 'object': {
            const { ok, parsed } = parseSerializedValue(value);
            return ok && isPlainObject(parsed);
        }
        case 'multistate':
...
function createRoleRuleMaps() {
    ...
    for (const [role, rule] of Object.entries(ROLE_RULES)) {
        // A trailing "*" (e.g. "button.mode.*") or a trailing "." marks a wildcard rule:
        // it matches any role starting with the text before the "*"/"." (inclusive).
        if (role.endsWith('*')) {
            prefixRules.push({ prefix: role.slice(0, -1), rule });
...
function validateStateCommon(stateId, common, collector, roleRuleMaps) {
    ...
    for (const [schemaKey, schema] of Object.entries(STATE_SCHEMA)) {
        ...
        if (schema.i18n) {
            const { errors: i18nErrors, warnings: i18nWarnings } = validateI18n(value, schemaKey);
            for (const msg of i18nErrors) {
                collector.error('E1007', `Object "${stateId}": ${msg}`);
            }
            for (const msg of i18nWarnings) {
                collector.warning('W1001', `Object "${stateId}": ${msg}`);
            }
        }
...
function validateObjectDump(objects, adapter, collector) {
    ...
    for (const [objectId, objectData] of entries) {
        ...
        for (const [schemaKey, descriptor] of Object.entries(ROOT_OBJECT_SCHEMA)) {
            const present = Object.prototype.hasOwnProperty.call(objectData, schemaKey);
            if (!present) {
                if (!descriptor.optional) {
                    collector.error('E3004', `Object "${objectId}": missing required key "${schemaKey}".`);
                }
                // optional + absent → silently skip
                continue;
            }
            // stateOnly keys are only permitted on objects of type 'state'
            if (descriptor.stateOnly && objectData.type !== 'state') {
                collector.error(
                    'E3005',
                    `Object "${objectId}": key "${schemaKey}" is only allowed on objects of type "state".`,
                );
                continue;
            }
            const actualType = getValueType(objectData[schemaKey]);
            if (descriptor.type && actualType !== descriptor.type) {
                collector.error(
                    'E3005',
                    `Object "${objectId}": key "${schemaKey}" must be type ${descriptor.type}, got ${actualType}.`,
                );
            }
            if (descriptor.matchesCommonType) {
                const commonType = objectData.common && objectData.common.type;
                if (commonType && !matchesCommonType(objectData[schemaKey], commonType)) {
                    collector.error(
                        'E3005',
                        `Object "${objectId}": key "${schemaKey}" must match common.type "${commonType}".`,
                    );
                }
            }
        }
    ...
    // --- Validate common.enums on all objects ---
    for (const [objectId, objectData] of entries) {
        if (isPlainObject(objectData.common) && objectData.common.enums !== undefined) {
            validateEnums(objectId, objectData.common.enums, collector);
        }
    }
