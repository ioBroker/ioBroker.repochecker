...
// Minimum number of words the LICENSE file must contain to be considered
// to hold the complete license text (instead of only a link or abbreviation).
...
async function checkLicenseFile(context) {
    ...
    if (data) {
        ...
        // The LICENSE file must always contain the complete license text
        // (no link or abbreviation is accepted). Validate by word count.
        const licenseWordCount = (data.trim().match(/\S+/g) || []).length;
        if (licenseWordCount < MIN_LICENSE_WORDS) {
            context.warnings.push(
                '[W7006] LICENSE file must contain the complete license text.',
            );
        } else {
            context.checks.push('LICENSE file contains the complete license text');
        }
        ...
        if (context.packageJson.license === 'MIT') {
            ...
            if (licenseYear > year) {
                ...
                if (m) {
                    context.errors.push(
                        `[E7005] Future year (${licenseYear}) found in LICENSE. Please use "Copyright (c) ${m[1]}-${year} ${getAuthor(context.packageJson.author)}" at the start of LICENSE`,
                    );
                } else {
                    context.errors.push(
                        `[E7005] Future year (${licenseYear}) found in LICENSE. Please use "Copyright (c) ${year} ${getAuthor(context.packageJson.author)}" at the start of LICENSE`,
                    );
                }
            } else if (!valid) {
                const m = text.match(/(\d\d\d\d)-\d\d\d\d/);
...
// [E7005] Future year (${licenseYear}) found in LICENSE. Please use correct year at the start of LICENSE.
// [W7006] LICENSE file must contain the complete license text.
