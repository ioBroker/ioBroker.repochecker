...
const { requestGithub, isGithubUrl } = require('./githubClient.js');
...
function stringifyError(error) {
    try {
        ...
        return JSON.stringify({
            ...
            stringifyError: serializationError?.message || `${serializationError}`,
        });
    }
}
...
async function downloadURL(url, binary) {
    ...
    try {
        ...
        const response = isGithubUrl(url) ? await requestGithub(url, options) : await axios(url, options);
...
async function getFilesList(context) {
    ...
    try {
        ...
        try {
            const response = await requestGithub(url2);
