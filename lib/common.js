    ...
    axios.interceptors.request.use(request => {
        ...
        try {
            if (request?.baseURL && request?.url) {
                requestUrl = new URL(request.url, request.baseURL).href;
            } else if (request?.url) {
        ...
        const hostName = requestUrl ? new URL(requestUrl).hostname.toLowerCase() : '';
        const isGitHubUrl = githubHostnames.has(hostName) || hostName.endsWith('.githubusercontent.com');
        if (isGitHubUrl) {
            request.headers = request.headers || {};
            request.headers.Authorization = 'Bearer ' + process.env.OWN_GITHUB_TOKEN;
        }
        return request;
    });
}
