...
const yamlLoadOptions = { schema: yaml.CORE_SCHEMA.withTags(yaml.mergeTag) };
...
/**
 * Loads a single YAML document using the compatibility settings required by js-yaml 5.
 * Empty or comment-only files return undefined, YAML merge keys remain supported,
 * and multi-document streams are rejected like yaml.load().
 *
 * @param {string} content YAML source
 * @returns {any} Parsed YAML document or undefined for empty/comment-only content
 */
function loadYamlDocument(content) {
    const docs = yaml.loadAll(content, undefined, yamlLoadOptions);
    if (!docs.length) {
        return undefined;
    }
...
exports.loadYamlDocument = loadYamlDocument;
