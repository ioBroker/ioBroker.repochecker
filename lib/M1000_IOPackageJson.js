        ...
        if (!(instanceObjects instanceof Array)) {
            ...
            instanceObjects.forEach((instanceObject, index) => {
                if (!instanceObject || typeof instanceObject !== 'object' || Array.isArray(instanceObject)) {
                    context.warnings.push(`[W1118] instanceObject[${index}] must be an object`);
                    return;
                }
                ...
                if (
                    ...
                    !instanceObject._id.trim()
                ) {
                    context.warnings.push(`[W1119] instanceObject[${index}] must define non-empty string "_id"`);
                }

                if (instanceObject.type !== undefined && !instanceObjectValidTypes.includes(instanceObject.type)) {
                    context.warnings.push(`[W1047] instanceObject type has an invalid type: ${instanceObject.type}`);
                }
                ...
                if (
                    ...
                    context.warnings.push(
                        `[W1120] instanceObject "${instanceObject._id || index}" with type "state" must have "common" object`,
                    );
                    return;
                }
                ...
                if (typeof instanceObject.common.type !== 'string') {
                    context.warnings.push(
                        `[W1048] instanceObject common.type has an invalid type! Expected "string", received  "${typeof instanceObject.common.type}"`,
                    );
                } else if (!instanceObjectStateTypes.includes(instanceObject.common.type)) {
                    context.warnings.push(
                        `[W1049] instanceObject common.type has an invalid value: ${instanceObject.common.type}`,
                    );
                }
                ...
                ['min', 'max', 'step'].forEach(key => {
                    if (instanceObject.common[key] !== undefined && typeof instanceObject.common[key] !== 'number') {
                        context.warnings.push(
                            `[W1121] instanceObject "${instanceObject._id || index}" common.${key} must be a number`,
                        );
                    }
                });
                ...
                if (instanceObject.common.unit !== undefined && typeof instanceObject.common.unit !== 'string') {
                    context.warnings.push(
                        `[W1122] instanceObject "${instanceObject._id || index}" common.unit must be a string`,
                    );
                }
                ...
                if (
                    ...
                    context.warnings.push(
                        `[W1123] instanceObject "${instanceObject._id || index}" common.desc must be a string or object`,
                    );
                }

                if (typeof instanceObject.common.read !== 'boolean') {
                    context.warnings.push(
                        `[W1124] instanceObject "${instanceObject._id || index}" common.read is mandatory and must be boolean`,
                    );
                }
                if (typeof instanceObject.common.write !== 'boolean') {
                    context.warnings.push(
                        `[W1125] instanceObject "${instanceObject._id || index}" common.write is mandatory and must be boolean`,
                    );
                }
                if (typeof instanceObject.common.role !== 'string' || !instanceObject.common.role.trim()) {
                    context.warnings.push(
                        `[W1126] instanceObject "${instanceObject._id || index}" common.role is mandatory and must be a non-empty string`,
                    );
                }
                if (
                    ...
                    context.warnings.push(
                        `[W1127] instanceObject "${instanceObject._id || index}" common.states must be number or string`,
                    );
                }

                if (instanceObject.common.defAck !== undefined && typeof instanceObject.common.defAck !== 'boolean') {
                    context.warnings.push(
                        `[W1128] instanceObject "${instanceObject._id || index}" common.defAck must be boolean`,
                    );
                }

                if (instanceObject.common.expert !== undefined && typeof instanceObject.common.expert !== 'boolean') {
                    context.warnings.push(
                        `[W1136] instanceObject "${instanceObject._id || index}" common.expert must be boolean`,
                    );
                }
                ...
                if (
                    ...
                    context.warnings.push(
                        `[W1129] instanceObject "${instanceObject._id || index}" common.workingID must be string`,
                    );
                }
                if (
                    ...
                    context.warnings.push(
                        `[W1130] instanceObject "${instanceObject._id || index}" common.def must match common.type "${instanceObject.common.type}"`,
                    );
                }
                ...
                Object.keys(instanceObject.common).forEach(commonKey => {
                    if (!validStateCommonKeys.has(commonKey)) {
                        context.warnings.push(
                            `[W1131] instanceObject "${instanceObject._id || index}" has unsupported common.${commonKey} attribute`,
                        );
                    }
                });
                ...
                if (typeof instanceObject.common.role === 'string') {
                    ...
                    if (!roleDefinition) {
                        context.warnings.push(
                            `[W1132] instanceObject "${instanceObject._id || index}" common.role "${instanceObject.common.role}" is not a known ioBroker state role`,
                        );
                        return;
                    }
                    ...
                    if (
                        ...
                        context.warnings.push(
                            `[W1133] instanceObject "${instanceObject._id || index}" role "${instanceObject.common.role}" expects common.type to be one of [${roleDefinition.types.join(', ')}], but found "${instanceObject.common.type}"`,
                        );
                    }
                    if (
                        ...
                        context.warnings.push(
                            `[W1134] instanceObject "${instanceObject._id || index}" role "${instanceObject.common.role}" expects common.read=${roleDefinition.read}`,
                        );
                    }
                    if (
                        ...
                        context.warnings.push(
                            `[W1135] instanceObject "${instanceObject._id || index}" role "${instanceObject.common.role}" expects common.write=${roleDefinition.write}`,
                        );
                    }
                }
            });
