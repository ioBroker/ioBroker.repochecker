...
async function checkIOPackageJson(context) {
    ...
    const hasTypeScriptMain = context.packageJson?.main?.endsWith('.ts');
    if (!context.ioPackageJson.common.onlyWWW && hasTypeScriptMain) {
        if (context.ioPackageJson.common.compact !== false) {
            context.errors.push(
