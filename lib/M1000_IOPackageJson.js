...
const { STATE_ROLE_RULES } = require('./config_StateRoles.js');
...
async function getIOPackageJson(context) {
    ...
    if (typeof context.ioPackageJson === 'string') {
        try {
            ...
            context.errors.push(`[E1000] Cannot parse io-package.json: ${e}`);
        }
    }
    return context;
}
...
let requiredJsControllerVersion = '6.0.11';

const recommendedAdminVersion = '7.6.20';
let requiredAdminVersion = '7.6.17'; // set to 6.17.14 if jsonConfig is used
const adaptersAllowedToUseNondeletable = [];
const generallyAllowedNotificationScopes = [];
const excludedNewsTranslationLanguages = new Set(['zh-cn']);
...
const blacklistIOPackageJson = {
    ...
    'common.adminTab.fa-icon': {
        msg: '"common.adminTab.fa-icon" is deprecated and ignored. Please remove from io-package.json.',
        err: true,
    },
};

const instanceObjectValidTypes = [
    'state',
    'channel',
    'device',
    'enum',
...
const instanceObjectStateTypes = [
    'array',
    'boolean',
    'file',
    'json',
...
function getNativeForChecks(ioPackageJson) {
    if (!ioPackageJson.native || typeof ioPackageJson.native !== 'object' || Array.isArray(ioPackageJson.native)) {
        return {};
    }

    if (ioPackageJson.common?.mode !== 'extension') {
        return ioPackageJson.native;
    }

    const nativeForChecks = { ...ioPackageJson.native };
    delete nativeForChecks.webInstance;
    return nativeForChecks;
}

function doesValueMatchStateType(value, stateType) {
    if (value === undefined) {
        return true;
    }
    if (stateType === 'mixed' || stateType === 'multistate') {
        return true;
    }
    if (stateType === 'array') {
    ...
    if (stateType === 'string' || stateType === 'file' || stateType === 'json') {
...
function isReadmeOrLicenseFile(path) {
    if (!path || typeof path !== 'string') {
        return false;
    }
    const normalized = path.replace(/^\/+/, '').toLowerCase();
    return normalized === 'readme.md' || normalized === 'license' || normalized === 'license.md';
}

function isCurrentRepositoryReadmeOrLicenseLink(context, link) {
    if (!link || typeof link !== 'string' || !context?.repository) {
        return false;
    }
    try {
        ...
        const pathParts = parsedUrl.pathname
            ...
            .filter(Boolean);
        const [owner, repo] = context.repository.toLowerCase().split('/');

        if (host === 'github.com') {
            if (pathParts.length < 5) {
                return false;
            }
            const [urlOwner, urlRepo, segment] = pathParts;
            if (urlOwner.toLowerCase() !== owner || urlRepo.toLowerCase() !== repo || segment !== 'blob') {
                return false;
            }
            return isReadmeOrLicenseFile(pathParts.slice(4).join('/'));
        }
        ...
        if (host === 'raw.githubusercontent.com') {
            ...
            if (urlOwner.toLowerCase() !== owner || urlRepo.toLowerCase() !== repo) {
                return false;
            }
            return isReadmeOrLicenseFile(pathParts.slice(3).join('/'));
        }
    } catch {
        return false;
    }
            ...
            if (validate.errors) {
                for (const error of validate.errors) {
                    ...
                    if (error.keyword === 'required') {
                        ...
                        const displayValues =
                            allowedValues.length > 5
                                ? `${allowedValues.slice(0, 5).join(', ')}, ...`
                                : allowedValues.join(', ');
                        errorMsg = `Property "${path}" should be one of: ${displayValues}`;
                    } else {
                        errorMsg = `Property "${path}" ${message}`;
                    }

                    context.errors.push(`[E1105] io-package.json schema validation error: ${errorMsg}`);
                }
            } else {
                context.errors.push('[E1105] io-package.json does not validate against schema');
            }
        }
    } catch (error) {
        common.debug(`Schema validation error: ${error.message}`);
        context.errors.push(`[E1108] io-package.json schema validation failed: ${error.message}`);
    }

    return context;
}

function checkNotificationsTranslations(obj, allowedLanguages) {
    if (!obj || typeof obj !== 'object') {
        return false;
    }
    if (Array.isArray(obj)) {
        return obj.some(item => checkNotificationsTranslations(item, allowedLanguages));
    }
    for (const key of Object.keys(obj)) {
        if (key === 'name' || key === 'description') {
            ...
            const missingLang = common.checkLanguages(value, allowedLanguages);
            if (missingLang.length) {
                return true;
            }
        } else if (checkNotificationsTranslations(obj[key], allowedLanguages)) {
            return true;
        }
    }
    return false;
...
function checkNotificationScopes(notifications, adapterName, allowedScopes) {
    const normalizedAdapterName = adapterName.toLowerCase();
    const normalizedAllowedScopes = new Set(allowedScopes.map(scope => scope.toLowerCase()));

    const missingScopeNotifications = [];
    const unsupportedScopeNotifications = [];
    ...
    for (let i = 0; i < notifications.length; i++) {
        ...
        if (!Object.prototype.hasOwnProperty.call(notification, 'scope')) {
            missingScopeNotifications.push(i);
            continue;
        }

        const normalizedScope =
            typeof notification.scope === 'string' ? notification.scope.toLowerCase() : String(notification.scope);
        if (normalizedScope !== normalizedAdapterName && !normalizedAllowedScopes.has(normalizedScope)) {
            unsupportedScopeNotifications.push({ index: i, scope: notification.scope });
        }
    }

    return { missingScopeNotifications, unsupportedScopeNotifications };
}

/**
 * Collect news entries where non-English language text equals the English text.
 *
 * @param {Record<string, Record<string, string>>} news common.news object from io-package.json
 * @returns {Record<string, string[]>} Object with language keys and affected version arrays
 */
function getUntranslatedNewsByLanguage(news) {
    const untranslatedNewsByLanguage = {};
    Object.keys(news).forEach(version => {
        const newsEntry = news[version];
        if (!newsEntry || typeof newsEntry !== 'object' || Array.isArray(newsEntry)) {
            return;
        }
        const englishText = newsEntry.en;
        if (typeof englishText !== 'string') {
            return;
        }
        Object.entries(newsEntry).forEach(([language, translation]) => {
            if (language === 'en' || excludedNewsTranslationLanguages.has(language)) {
                return;
            }
            if (typeof translation === 'string' && translation === englishText) {
                const versions = untranslatedNewsByLanguage[language] || [];
                versions.push(version);
                untranslatedNewsByLanguage[language] = versions;
            }
        });
    });
    return untranslatedNewsByLanguage;
}

/**
 * Determine the newest release from a common.news object using semver ordering.
 *
 * @param {Record<string, Record<string, string>>} news common.news object from io-package.json
 * @returns {string | undefined} the highest version found, or undefined if none is valid
 */
function getNewestNewsVersion(news) {
    let newestVersion;
    Object.keys(news).forEach(version => {
        try {
            if (newestVersion === undefined || compareVersions.compareVersions(version, newestVersion) > 0) {
                newestVersion = version;
            }
        } catch {
            // ignore malformed version strings, they are reported by E1075
        }
    });
...
/**
 * Collect languages whose news translations are significantly shorter than the English text.
 * A translation counts as "too short" if it is more than 40% shorter than the English original.
 *
 * @param {Record<string, Record<string, string>>} news common.news object from io-package.json
 * @returns {Record<string, string[]>} Object with language keys and affected version arrays
 */
function getShortNewsTranslationLanguages(news) {
    const shortNewsByLanguage = {};
    Object.keys(news).forEach(version => {
        const newsEntry = news[version];
        if (!newsEntry || typeof newsEntry !== 'object' || Array.isArray(newsEntry)) {
            return;
        ...
        Object.entries(newsEntry).forEach(([language, translation]) => {
            ...
            if (typeof translation === 'string' && translation.length < englishText.length * 0.6) {
                const versions = shortNewsByLanguage[language] || [];
                versions.push(version);
                shortNewsByLanguage[language] = versions;
            }
        });
    });
    return shortNewsByLanguage;
}

async function checkIOPackageJson(context) {
    console.log('\n[E1000 - E1999] checking io-package.json');

    let encryptedNativeHasDotIds = false;
    ...
    if (!context.ioPackageJson.native) {
        ...
        const nativeForChecks = getNativeForChecks(context.ioPackageJson);
        ...
        const nativeKeys = Object.keys(nativeForChecks) || [];
        ...
        if (suspiciousKeys.length) {
            suspiciousKeys = suspiciousKeys.filter(key => typeof nativeForChecks[key] !== 'boolean');
            common.debug(`Suspicious key test: suspiciousKeys (filtered): ${suspiciousKeys}`);
        }
        if (suspiciousKeys.length) {
            const missingProtected = context.ioPackageJson.protectedNative
                ? suspiciousKeys.filter(
                      key => !context.ioPackageJson.protectedNative.includes(key),
                  )
                : [...suspiciousKeys];
            context.reportedMissingProtectedNativeKeys = missingProtected;
            ...
            const missingEncrypted = context.ioPackageJson.encryptedNative
                ? suspiciousKeys.filter(
                      key => !context.ioPackageJson.encryptedNative.includes(key),
                  )
                : [...suspiciousKeys];
            context.reportedMissingEncryptedNativeKeys = missingEncrypted;
            if (missingEncrypted.length) {
                context.warnings.push(
                    `[W1074] Potential sensitive data "${missingEncrypted.join(', ')}" not listed at "encryptedNative" in io-package.json`,
                );
            }
        }
        ...
        if (valid) {
            ...
            for (const key of [
                ...
                if (nativeForChecks[key] === undefined) {
                    // Check if the key contains a dot, indicating an array element path
                    if (key.includes('.')) {
                        if (
                            Array.isArray(context.ioPackageJson.encryptedNative) &&
                            context.ioPackageJson.encryptedNative.includes(key)
                        ) {
                            // encryptedNative dot key: supported since js-controller 7.0.7
                            encryptedNativeHasDotIds = true;
                        } else {
                            // protectedNative-only dot key: not yet supported
        ...
        // Check for example/template keys that should be removed
        // Note: actual error E5040 is raised in M5000_Code.js after checking if the keys are used in admin UI files
        const exampleKeys = ['option1', 'option2'];
        context.nativeExampleKeys = exampleKeys.filter(key =>
            Object.prototype.hasOwnProperty.call(nativeForChecks, key),
        );
    }
    ...
    if (context.ioPackageJson.common.jsonConfig) {
        if (context.ioPackageJson.common.adminUI) {
            context.warnings.push(
                '[W1109] "common.jsonConfig" replaced by "common.adminUI.config:json" at io-package.json. Please remove.',
            );
        } else {
            context.errors.push(
                '[E1110] "common.jsonConfig" is deprecated and must be replaced by "common.adminUI.config:json" configuration at io-package.json.',
            );
        }
    } else {
        context.checks.push('"common.jsonConfig" does not exist in io-package.json');
    }
    ...
    const hasTypeScriptMainEntry = context.packageJson?.main?.endsWith('.ts');
    if (!context.ioPackageJson.common.onlyWWW && hasTypeScriptMainEntry) {
        if (context.ioPackageJson.common.compact === undefined || context.ioPackageJson.common.compact === true) {
            context.errors.push(
                '[E1143] package.json "main" points to a TypeScript file. Please set "common.compact": false in io-package.json.',
            );
        } else {
            context.checks.push('"common.compact" found in io-package.json');
        }
    } else if (!context.ioPackageJson.common.compact && !context.ioPackageJson.common.onlyWWW) {
    ...
    if (
        context.ioPackageJson.common.adminUI?.config === 'none' &&
        context.ioPackageJson.native &&
        Object.keys(getNativeForChecks(context.ioPackageJson)).length > 0
    ) {
        context.warnings.push(
            '[W1113] "native" config object will not be used when "common.adminUI.config" is set to "none". Please remove native properties from io-package.json.',
        );
    }
    if (
    ...
    if (!context.ioPackageJson.common.licenseInformation) {
        ...
        if (
            ...
            const licenseInformationLink = context.ioPackageJson.common?.licenseInformation?.link;
            if (
                ['paid', 'commercial', 'limited'].includes(context.ioPackageJson.common?.licenseInformation?.type) &&
                !licenseInformationLink
            ) {
                context.warnings.push('[W1071] "common.licenseInformation.link" is required for non-free adapters');
            } else if (licenseInformationLink) {
                context.checks.push('"common.licenseInformation.link" is valid in io-package.json');

                if (isCurrentRepositoryReadmeOrLicenseLink(context, licenseInformationLink)) {
                    context.warnings.push(
                        '[W1137] "common.licenseInformation.link" should not point to this repository\'s README.md or LICENSE file',
                    );
                }

                if (!((await common.downloadURL(licenseInformationLink, false)) || '').trim()) {
                    context.warnings.push('[W1138] "common.licenseInformation.link" should point to a readable URL');
                } else {
                    context.checks.push('"common.licenseInformation.link" points to a readable URL');
                }
            }
        }
    ...
    if (!context.ioPackageJson.common.mode) {
        ...
        if (!context.cfg.allowedModes[context.ioPackageJson.common.mode]) {
            ...
            // Check schedule attribute - not supported for daemon adapters
            if (context.ioPackageJson.common.mode === 'daemon' && context.ioPackageJson.common.schedule !== undefined) {
                if (context.ioPackageJson.common.schedule === '') {
                    context.warnings.push(
                        '[S1114] "common.schedule" is unused for daemon adapters. Please remove the attribute from io-package.json.',
                    );
                } else {
                    context.warnings.push(
                        '[W1114] "common.schedule" is not supported for daemon adapters and should be removed from io-package.json.',
                    );
                }
            }
    ...
    if (!context.ioPackageJson.common.news) {
        ...
        if (!context.ioPackageJson.common.news[context.ioPackageJson.common.version]) {
            if (!common.isAlphaVersion(context.ioPackageJson.common.version)) {
                context.errors.push(
        ...
        const untranslatedNewsByLanguage = getUntranslatedNewsByLanguage(context.ioPackageJson.common.news);
        const untranslatedLanguages = Object.keys(untranslatedNewsByLanguage).sort();
        // Only report when more than two languages are identical to the English text,
        // but always report if German (de) is identical to English.
        if (untranslatedLanguages.length > 2 || untranslatedLanguages.includes('de')) {
            const untranslatedDetails = untranslatedLanguages
                .map(language => `${language}(${untranslatedNewsByLanguage[language].sort().join(', ')})`)
                .join('; ');
            const newestVersion = getNewestNewsVersion(context.ioPackageJson.common.news);
            const newestAffected =
                newestVersion !== undefined &&
                untranslatedLanguages.some(language => untranslatedNewsByLanguage[language].includes(newestVersion));
            if (newestAffected) {
                // The newest release is affected - this is an error.
                context.errors.push(
                    `[E1144] Untranslated "common.news" entries detected (identical to English text): ${untranslatedDetails}.`,
                );
            } else {
                // Only older releases are affected - log a single warning.
                context.warnings.push(
                    `[W1144] Untranslated "common.news" entries detected (identical to English text): ${untranslatedDetails}.`,
                );
            }
        }

        // Warn (once) if more than two languages have news translations significantly shorter than the English text.
        const shortNewsByLanguage = getShortNewsTranslationLanguages(context.ioPackageJson.common.news);
        const shortNewsLanguages = Object.keys(shortNewsByLanguage).sort();
        if (shortNewsLanguages.length > 2) {
            const shortNewsDetails = shortNewsLanguages
                .map(language => `${language}(${shortNewsByLanguage[language].sort().join(', ')})`)
                .join('; ');
            context.warnings.push(
                `[W1145] Some "common.news" translations are significantly shorter than the English text: ${shortNewsDetails}. Please check the translations.`,
            );
        }
    ...
    if (context.ioPackageJson.instanceObjects) {
        ...
        if (!(instanceObjects instanceof Array)) {
            context.warnings.push('[W1046] instanceObjects must be an Array in io-package.json');
        } else {
            const validStateCommonKeys = new Set([
                'min',
                'max',
                'step',
                'unit',
            ...
            instanceObjects.forEach((instanceObject, index) => {
                if (!instanceObject || typeof instanceObject !== 'object' || Array.isArray(instanceObject)) {
                    context.warnings.push(`[W1118] io-package.json: instanceObject[${index}] must be an object`);
                    return;
                }

                if (
                    (instanceObject._id === undefined ||
                    typeof instanceObject._id !== 'string' ||
                    !instanceObject._id.trim()) && (instanceObject.type !== 'meta')
                ) {
                    context.warnings.push(`[W1119] io-package.json: instanceObject[${index}] must define non-empty string "_id"`);
                }

                if (instanceObject.type !== undefined && !instanceObjectValidTypes.includes(instanceObject.type)) {
                    context.warnings.push(`[W1047] io-package.json: instanceObject type has an invalid type: ${instanceObject.type}`);
                }

                if (instanceObject.type !== 'state') {
                    return;
                }

                if (
                    !instanceObject.common ||
                    typeof instanceObject.common !== 'object' ||
                    Array.isArray(instanceObject.common)
                ) {
                    context.warnings.push(
                        `[W1120] io-package.json: instanceObject "${instanceObject._id || index}" with type "state" must have "common" object`,
                    );
                    return;
                }
                ...
                if (typeof instanceObject.common.type !== 'string') {
                    context.warnings.push(
                        `[W1048] io-package.json: instanceObject common.type has an invalid type! Expected "string", received  "${typeof instanceObject.common.type}"`,
                    );
                } else if (!instanceObjectStateTypes.includes(instanceObject.common.type)) {
                    context.warnings.push(
                        `[W1049] io-package.json: instanceObject common.type has an invalid value: ${instanceObject.common.type}`,
                    );
                }

                ['min', 'max', 'step'].forEach(key => {
                    if (instanceObject.common[key] !== undefined && typeof instanceObject.common[key] !== 'number') {
                        context.warnings.push(
                            `[W1121] io-package.json: instanceObject "${instanceObject._id || index}" common.${key} must be a number`,
                        );
                    }
                });

                if (instanceObject.common.unit !== undefined && typeof instanceObject.common.unit !== 'string') {
                    context.warnings.push(
                        `[W1122] io-package.json: instanceObject "${instanceObject._id || index}" common.unit must be a string`,
                    );
                }

                if (
                    instanceObject.common.desc !== undefined &&
                    typeof instanceObject.common.desc !== 'string' &&
                    (typeof instanceObject.common.desc !== 'object' ||
                        instanceObject.common.desc === null ||
                        Array.isArray(instanceObject.common.desc))
                ) {
                    context.warnings.push(
                        `[W1123] io-package.json: instanceObject "${instanceObject._id || index}" common.desc must be a string or object`,
                    );
                }

                if (typeof instanceObject.common.read !== 'boolean') {
                    context.warnings.push(
                        `[W1124] io-package.json: instanceObject "${instanceObject._id || index}" common.read is mandatory and must be boolean`,
                    );
                }
                if (typeof instanceObject.common.write !== 'boolean') {
                    context.warnings.push(
                        `[W1125] io-package.json: instanceObject "${instanceObject._id || index}" common.write is mandatory and must be boolean`,
                    );
                }
                if (typeof instanceObject.common.role !== 'string' || !instanceObject.common.role.trim()) {
                    context.warnings.push(
                        `[W1126] io-package.json: instanceObject "${instanceObject._id || index}" common.role is mandatory and must be a non-empty string`,
                    );
                }
                if (
                    instanceObject.common.states !== undefined &&
                    (typeof instanceObject.common.states !== 'object')
                ) {
                    context.warnings.push(
                        `[W1127] io-package.json: instanceObject "${instanceObject._id || index}" common.states must be an object`,
                    );
                }

                if (instanceObject.common.defAck !== undefined && typeof instanceObject.common.defAck !== 'boolean') {
                    context.warnings.push(
                        `[W1128] io-package.json: instanceObject "${instanceObject._id || index}" common.defAck must be boolean`,
                    );
                }

                if (instanceObject.common.expert !== undefined && typeof instanceObject.common.expert !== 'boolean') {
                    context.warnings.push(
                        `[W1136] io-package.json: instanceObject "${instanceObject._id || index}" common.expert must be boolean`,
                    );
                }

                if (
                    instanceObject.common.workingID !== undefined &&
                    typeof instanceObject.common.workingID !== 'string'
                ) {
                    context.warnings.push(
                        `[W1129] io-package.json: instanceObject "${instanceObject._id || index}" common.workingID must be string`,
                    );
                }
                if (
                    typeof instanceObject.common.type === 'string' &&
                    !doesValueMatchStateType(instanceObject.common.def, instanceObject.common.type)
                ) {
                    context.warnings.push(
                        `[W1130] io-package.json: instanceObject "${instanceObject._id || index}" common.def must match common.type "${instanceObject.common.type}"`,
                    );
                }

                Object.keys(instanceObject.common).forEach(commonKey => {
                    if (!validStateCommonKeys.has(commonKey)) {
                        context.warnings.push(
                            `[W1131] io-package.json: instanceObject "${instanceObject._id || index}" has unsupported common.${commonKey} attribute`,
                        );
                    }
                });

                if (typeof instanceObject.common.role === 'string') {
                    // Fast path: direct key lookup (covers the vast majority of roles)
                    let roleDefinition = STATE_ROLE_RULES[instanceObject.common.role];

                    // Slow path: scan for wildcard patterns if no direct match was found
                    if (!roleDefinition) {
                        const wildcardKeys = Object.keys(STATE_ROLE_RULES).filter(k => k.includes('*'));
                        for (const pattern of wildcardKeys) {
                            ...
                            if (new RegExp(`^${regexStr}$`).test(instanceObject.common.role)) {
                                roleDefinition = STATE_ROLE_RULES[pattern];
                                break;
                            }
                        }
                    }
                    ...
                    if (!roleDefinition) {
                        context.warnings.push(
                            `[W1132] io-package.json: instanceObject "${instanceObject._id || index}" common.role "${instanceObject.common.role}" is not a known ioBroker state role`,
                        );
                        return;
                    }

                    if (!Object.keys(roleDefinition).length) {
                        return;
                    }

                    if (roleDefinition.types && !roleDefinition.types.includes(instanceObject.common.type)) {
                        context.warnings.push(
                            `[W1133] io-package.json: instanceObject "${instanceObject._id || index}" role "${instanceObject.common.role}" expects common.type to be one of [${roleDefinition.types.join(', ')}], but found "${instanceObject.common.type}"`,
                        );
                    }
                    if ('read' in roleDefinition && instanceObject.common.read !== roleDefinition.read) {
                        context.warnings.push(
                            `[W1134] io-package.json: instanceObject "${instanceObject._id || index}" role "${instanceObject.common.role}" expects common.read=${roleDefinition.read}`,
                        );
                    }
                    if ('write' in roleDefinition && instanceObject.common.write !== roleDefinition.write) {
                        context.warnings.push(
                            `[W1135] io-package.json: instanceObject "${instanceObject._id || index}" role "${instanceObject.common.role}" expects common.write=${roleDefinition.write}`,
                        );
                    }
                }

                if (instanceObject.common.color !== undefined && typeof instanceObject.common.color !== 'string') {
                    context.warnings.push(
                        `[W1140] io-package.json: instanceObject "${instanceObject._id || index}" common.color must be a string`,
                    );
                }

                if (instanceObject.common.icon !== undefined && typeof instanceObject.common.icon !== 'string') {
                    context.warnings.push(
                        `[W1141] io-package.json: instanceObject "${instanceObject._id || index}" common.icon must be a string`,
                    );
                }
            });
        }
    ...
    if (encryptedNativeHasDotIds) {
        requiredJsControllerVersion = common.maxVersion(requiredJsControllerVersion, '7.0.7');
    }

    // Store the resolved version in context for use by later check modules (e.g. M5000_Code)
    context.currentJsControllerVersion = currentJsControllerVersion;
    ...
    // admin itself does never require admin
    if (context.adapterName === 'admin') {
        requireAdmin = false;
    }

    common.debug(`admin dependency is ${requireAdmin ? '' : 'not'}required`);

    // no longer needed as minimum admin is 7.x.x now
        ...
        if (!(context.ioPackageJson.common.globalDependencies instanceof Array)) {
            ...
            if (adminDependency) {
                ...
                if (!adminDependency['admin'].startsWith('>=')) {
                    context.errors.push(
                        `[E1089] common.globalDependencies "admin" dependency should always allow future versions (>=x.x.x) - recommended: {"admin": ">=${recommendedAdminVersion}"}.`,
                    );
                } else {
    ...
    if (context.ioPackageJson.common.nondeletable === true) {
        if (adaptersAllowedToUseNondeletable.includes(context.adapterName)) {
            context.checks.push(
                '"common.nondeletable=true" found in io-package.json for adapter allowed to use this flag',
            );
        } else {
            context.errors.push(
                '[E1084] "common.nondeletable=true" detected in io-package.json. This flag is reserved and must be removed.',
            );
        }
    } else {
        context.checks.push('"common.nondeletable" is not set to true in io-package.json');
    }
    ...
    if (context.ioPackageJson.common.plugins && typeof context.ioPackageJson.common.plugins === 'object') {
        const pluginNames = Object.keys(context.ioPackageJson.common.plugins);
        const allDeps = {
            ...(context.packageJson.dependencies || {}),
        };
        const missingPluginDeps = pluginNames
            .filter(name => name !== 'sentry')
            .filter(name => !allDeps[`@iobroker/plugin-${name}`]);
        if (missingPluginDeps.length > 0) {
            for (const pluginName of missingPluginDeps) {
                context.warnings.push(
                    `[W1115] Plugin "${pluginName}" listed in io-package.json but "@iobroker/plugin-${pluginName}" is not listed as dependency in package.json.`,
                );
            }
        } else {
            context.checks.push(
                'All plugins in "common.plugins" have corresponding entries in package.json dependencies.',
            );
        }

        // W1116: if sentry plugin is used, no explicit @sentry/* packages should be in dependencies
        if (pluginNames.includes('sentry')) {
            const sentryDeps = Object.keys(context.packageJson.dependencies || {}).filter(dep =>
                dep.startsWith('@sentry/'),
            );
            if (sentryDeps.length > 0) {
                context.warnings.push(
                    `[W1116] Sentry plugin is configured in io-package.json but "@sentry/*" packages (${sentryDeps.join(', ')}) are listed as dependencies in package.json. When using the sentry plugin, explicit @sentry/* dependencies should not be added.`,
                );
            } else {
                context.checks.push('Sentry plugin used and no explicit @sentry/* dependencies found in package.json.');
            }
        }
    } else {
        context.checks.push('"common.plugins" not found in io-package.json, skipping check');
    }

    if (context.ioPackageJson.notifications) {
        if (!Array.isArray(context.ioPackageJson.notifications)) {
            context.errors.push('[E1112] "notifications" in io-package.json must be an array');
        } else {
            if (checkNotificationsTranslations(context.ioPackageJson.notifications, context.cfg.allowedLanguages)) {
                context.errors.push(
                    `[E1139] "notifications" in io-package.json must be translated into all supported languages (${context.cfg.allowedLanguages.join(', ')})`,
                );
            } else {
                context.checks.push('"notifications" in io-package.json are translated into all supported languages');
            }

            const { missingScopeNotifications, unsupportedScopeNotifications } = checkNotificationScopes(
                context.ioPackageJson.notifications,
                context.adapterName,
                generallyAllowedNotificationScopes,
            );

            for (const notificationIndex of missingScopeNotifications) {
                context.errors.push(
                    `[E1137] "notifications[${notificationIndex}].scope" is missing in io-package.json`,
                );
            }

            for (const notification of unsupportedScopeNotifications) {
                context.warnings.push(
                    `[W1142] "notifications[${notification.index}].scope" has unsupported value "${notification.scope}" in io-package.json. Use "${context.adapterName.toLowerCase()}" or a generally allowed scope.`,
                );
            }
        }
    } else {
        context.checks.push('"notifications" not found in io-package.json, skipping check');
    }
...
// [1103] Encryption of array elements "${key}" is not yet available (protectedNative-only dot keys)
// [1103] encryptedNative dot keys are supported since js-controller 7.0.7 and trigger E1057 if the version requirement is not met
...
// [1109] "common.jsonConfig" is deprecated and has been replaced by "common.adminUI" at io-package.json. Please remove.
// [1110] "common.jsonConfig" is deprecated and must be replaced by "common.adminUI" configuration at io-package.json.
// [1111] -- moved to M5000_Code.js as [E5040] (check deferred to allow scanning admin UI files)
// [1112] "notifications" in io-package.json must be an array
// [1113] "native" config object will not be used when "common.adminUI.config" is set to "none". Please remove native properties from io-package.json.
// [1114] "common.schedule" is not supported / unused for daemon adapters and should be removed from io-package.json.
// [1115] Plugin "${pluginName}" is listed in "common.plugins" of io-package.json but "@iobroker/plugin-${pluginName}" is not listed as a dependency in package.json. Please add it.
// [1116] Sentry plugin is configured in io-package.json but "@sentry/*" packages (${sentryDeps.join(', ')}) are listed as dependencies in package.json. When using the sentry plugin, explicit @sentry/* dependencies should not be added.
// [W1118] io-package.json: instanceObject[${index}] must be an object
// [W1119] io-package.json: instanceObject[${index}] must define non-empty string "_id"
// [W1120] io-package.json: instanceObject "${instanceObject._id || index}" with type "state" must have "common" object
// [W1121] io-package.json: instanceObject "${instanceObject._id || index}" common.${key} must be a number
// [W1122] io-package.json: instanceObject "${instanceObject._id || index}" common.unit must be a string
// [W1123] io-package.json: instanceObject "${instanceObject._id || index}" common.desc must be a string or object
...
// [W1125] io-package.json: instanceObject "${instanceObject._id || index}" common.write is mandatory and must be boolean
// [W1126] io-package.json: instanceObject "${instanceObject._id || index}" common.role is mandatory and must be a non-empty string
// [W1127] io-package.json: instanceObject "${instanceObject._id || index}" common.states must be number or string
// [W1128] io-package.json: instanceObject "${instanceObject._id || index}" common.defAck must be boolean
// [W1129] io-package.json: instanceObject "${instanceObject._id || index}" common.workingID must be string
// [W1130] io-package.json: instanceObject "${instanceObject._id || index}" common.def must match common.type "${instanceObject.common.type}"
// [W1131] io-package.json: instanceObject "${instanceObject._id || index}" has unsupported common.${commonKey} attribute
...
// [W1136] io-package.json: instanceObject "${instanceObject._id || index}" common.expert must be boolean
// [W1137] "common.licenseInformation.link" should not point to this repository's README.md or LICENSE file
// [W1138] "common.licenseInformation.link" should point to a readable URL
// [E1137] "notifications[${notificationIndex}].scope" is missing in io-package.json
// [E1139] "notifications" in io-package.json must be translated into all supported languages
// [W1140] io-package.json: instanceObject common.color must be a string
// [W1141] io-package.json: instanceObject common.icon must be a string
// [W1142] "notifications[${notification.index}].scope" has unsupported value "${notification.scope}" in io-package.json.
// [E1143] package.json "main" points to a TypeScript file. Please set "common.compact": false in io-package.json.
// [E1144] Untranslated "common.news" entries detected (identical to English text): ${language(version1, version2)}; ...  (error if newest release affected, else W1144)
// [W1145] Some "common.news" translations are significantly shorter than the English text: ${language(version1, version2)}; ... Please check the translations.
