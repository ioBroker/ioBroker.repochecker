...
const stateRolesUrl = 'https://raw.githubusercontent.com/ioBroker/ioBroker.docs/master/docs/en/dev/stateroles.md';
let stateRoleDefinitionsCache;

function mergeTypeConstraint(base, next) {
    if (!base || !next) {
        return undefined;
    }
...
function mergeBooleanConstraint(base, next) {
    if (base === next) {
        return base;
    }
    if (base === 'any' || next === 'any') {
        return 'any';
    }
    return 'any';
}

function parseStateRoleDefinitions(markdown) {
    const profileBySection = {
        '### Common': { types: undefined, read: 'any', write: 'any' },
        '### Sensor (booleans, read-only)': { types: ['boolean'], read: true, write: false },
        '### Buttons (booleans, write-only)': { types: ['boolean'], read: false, write: true },
        '### Buttons as sensor': { types: ['boolean'], read: true, write: false },
        '### Values (numbers, read-only)': { types: ['number'], read: true, write: false },
        '### Indicators (boolean, read-only)': { types: ['boolean'], read: true, write: false },
        '### Levels (numbers, read-write)': { types: ['number'], read: true, write: true },
        '### Switches (booleans, read-write)': { types: ['boolean'], read: true, write: true },
        '### Health': { types: ['number'], read: true, write: false },
        '### Others': { types: undefined, read: 'any', write: 'any' },
    };

    let currentProfile = profileBySection['### Others'];
    const definitions = {};
    for (const rawLine of markdown.split(/\r?\n/)) {
        const line = rawLine.trim();
        if (profileBySection[line]) {
            currentProfile = profileBySection[line];
            continue;
        }
        ...
        const comment = roleMatch[2] || '';
        if (role.includes('*') || role.startsWith('~~') || role.endsWith('~~')) {
            continue;
        }

        const entry = {
            types: currentProfile.types ? [...currentProfile.types] : undefined,
            read: currentProfile.read,
            write: currentProfile.write,
        };
        const inlineTypes = [
            ...comment.matchAll(/common\.type\s*=\s*(array|boolean|file|json|mixed|multistate|number|object|string)/g),
        ].map(match => match[1]);
        if (inlineTypes.length) {
            entry.types = [...new Set(inlineTypes)];
        }

        if (definitions[role]) {
            definitions[role] = {
                types: mergeTypeConstraint(definitions[role].types, entry.types),
                read: mergeBooleanConstraint(definitions[role].read, entry.read),
                write: mergeBooleanConstraint(definitions[role].write, entry.write),
            };
        } else {
...
async function getStateRoleDefinitions(context) {
    if (stateRoleDefinitionsCache) {
        return stateRoleDefinitionsCache;
    }

    const markdown = await common.downloadURL(stateRolesUrl, false);
    if (!markdown || typeof markdown !== 'string') {
        context.warnings.push(
            `[W1117] Could not download ${stateRolesUrl} for validation of instanceObjects common.role`,
        );
        stateRoleDefinitionsCache = {};
        return stateRoleDefinitionsCache;
    }

    stateRoleDefinitionsCache = parseStateRoleDefinitions(markdown);
    return stateRoleDefinitionsCache;
}

function normalizeStateRoleForLookup(role) {
    if (!role.includes('.setting.')) {
        return role.endsWith('.setting') ? role.slice(0, -8) : role;
    }
    return role.replace('.setting.', '.');
}
...
async function checkIOPackageJson(context) {
    ...
    if (context.ioPackageJson.instanceObjects) {
        ...
        if (!(instanceObjects instanceof Array)) {
            ...
            const stateRoleDefinitions = await getStateRoleDefinitions(context);
            ...
            instanceObjects.forEach((instanceObject, index) => {
                ...
                if (
                    ...
                    (typeof instanceObject.common.states !== 'number' &&
                        typeof instanceObject.common.states !== 'string')
                ) {
                    context.warnings.push(
                ...
                if (typeof instanceObject.common.role === 'string') {
                    const roleForLookup = normalizeStateRoleForLookup(instanceObject.common.role);
                    const roleDefinition = stateRoleDefinitions[roleForLookup];
                    if (!roleDefinition) {
                        context.warnings.push(
                    ...
                    if (
                        ...
                        typeof instanceObject.common.type === 'string' &&
                        !roleDefinition.types.includes(instanceObject.common.type)
                    ) {
                        context.warnings.push(
                    ...
                    if (
                        typeof instanceObject.common.read === 'boolean' &&
                        roleDefinition.read !== 'any' &&
                        instanceObject.common.read !== roleDefinition.read
                    ) {
                        context.warnings.push(
                    ...
                    if (
                        typeof instanceObject.common.write === 'boolean' &&
                        roleDefinition.write !== 'any' &&
                        instanceObject.common.write !== roleDefinition.write
                    ) {
                        context.warnings.push(
...
// [W1117] Could not download stateRoles URL for validation of instanceObjects common.role
