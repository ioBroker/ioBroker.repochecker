    ...
    if (context.ioPackageJson.common.plugins && typeof context.ioPackageJson.common.plugins === 'object') {
        ...
        if (missingPluginDeps.length > 0) {
            for (const pluginName of missingPluginDeps) {
                context.warnings.push(
                    `[W1115] Plugin "${pluginName}" is listed in "common.plugins" of io-package.json but "@iobroker/plugin-${pluginName}" is not listed as a dependency in package.json. Please add it.`,
                );
            }
        } else {
            context.checks.push(
