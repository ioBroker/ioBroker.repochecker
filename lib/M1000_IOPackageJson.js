        ...
        if (!(instanceObjects instanceof Array)) {
            ...
            instanceObjects.forEach((instanceObject, index) => {
                ...
                if (instanceObject.common.color !== undefined && typeof instanceObject.common.color !== 'string') {
                    context.warnings.push(
                        `[W1136] io-package.json: instanceObject "${instanceObject._id || index}" common.color must be a string`,
                    );
                }

                if (instanceObject.common.icon !== undefined && typeof instanceObject.common.icon !== 'string') {
                    context.warnings.push(
                        `[W1136] io-package.json: instanceObject "${instanceObject._id || index}" common.icon must be a string`,
                    );
                }
    ...
    if (context.ioPackageJson.notifications) {
        if (!Array.isArray(context.ioPackageJson.notifications)) {
            ...
            if (checkNotificationsTranslations(context.ioPackageJson.notifications, context.cfg.allowedLanguages)) {
                context.errors.push(
                    `[E1112] "notifications" in io-package.json must be translated into all supported languages (${context.cfg.allowedLanguages.join(', ')})`,
                );
            } else {
            ...
            for (const notification of unsupportedScopeNotifications) {
                context.warnings.push(
                    `[W1138] "notifications[${notification.index}].scope" has unsupported value "${notification.scope}" in io-package.json. Use "${context.adapterName.toLowerCase()}" or a generally allowed scope.`,
                );
            }
        }
    } else {
...
// [1112] "notifications" in io-package.json must be translated into all supported languages
...
// [1137] "common.licenseInformation.link" should not point to this repository's README.md or LICENSE file
// [1138] "common.licenseInformation.link" should point to a readable URL
// [1137] "notifications[${notificationIndex}].scope" is missing in io-package.json
// [1138] "notifications[${notification.index}].scope" has unsupported value "${notification.scope}" in io-package.json. Use "${context.adapterName.toLowerCase()}" or a generally allowed scope.
