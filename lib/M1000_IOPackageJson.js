 ...
 * A translation counts as "too short" if it is more than 20% shorter than the English original.
 *
 * @param {Record<string, Record<string, string>>} news common.news object from io-package.json
 * @returns {string[]} sorted list of languages with at least one significantly shorter translation
 */
function getShortNewsTranslationLanguages(news) {
    const shortLanguages = new Set();
    Object.keys(news).forEach(version => {
        const newsEntry = news[version];
        if (!newsEntry || typeof newsEntry !== 'object' || Array.isArray(newsEntry)) {
            return;
        ...
        Object.entries(newsEntry).forEach(([language, translation]) => {
            ...
            if (typeof translation === 'string' && translation.length < englishText.length * 0.8) {
                shortLanguages.add(language);
            }
        });
    });
    return [...shortLanguages].sort();
}
...
async function checkIOPackageJson(context) {
    ...
    if (!context.ioPackageJson.common.news) {
        ...
        const shortNewsTranslationLanguages = getShortNewsTranslationLanguages(context.ioPackageJson.common.news);
        if (shortNewsTranslationLanguages.length > 2) {
            context.warnings.push(
                `[W1145] Some "common.news" translations are significantly shorter than the English text (${shortNewsTranslationLanguages.join(', ')}). Please check the translations.`,
            );
        }
...
// [W1145] Some "common.news" translations are significantly shorter than the English text (${languages}). Please check the translations.
