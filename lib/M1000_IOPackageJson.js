...
async function checkIOPackageJson(context) {
    ...
    if (!context.ioPackageJson.common.news) {
        ...
        Object.keys(context.ioPackageJson.common.news).forEach(version => {
            const newsEntry = context.ioPackageJson.common.news[version];
            ...
            Object.entries(newsEntry).forEach(([language, translation]) => {
                ...
                if (typeof translation === 'string' && translation === englishText) {
                    untranslatedNewsByLanguage[language] = untranslatedNewsByLanguage[language] || [];
                    untranslatedNewsByLanguage[language].push(version);
                }
            });
        });
        ...
        if (untranslatedLanguages.length) {
            const untranslatedDetails = untranslatedLanguages
                .map(language => `${language}(${[...new Set(untranslatedNewsByLanguage[language])].sort().join(', ')})`)
                .join('; ');
            context.errors.push(
...
// [E1143] package.json "main" points to a TypeScript file. Please set "common.compact": false in io-package.json.
// [E1144] Untranslated "common.news" entries detected (identical to English text): ${language(versionList)}.
