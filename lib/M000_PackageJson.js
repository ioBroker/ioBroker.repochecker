...
async function checkPackageJson(context) {
    ...
    if (!context.packageJson.version) {
        ...
        if (!validateReleaseFormat(context.packageJson.version)) {
            context.errors.push(
                `[E061] Invalid version format "${context.packageJson.version}". Version must follow semver format: MAJOR.MINOR.PATCH or MAJOR.MINOR.PATCH-PRERELEASE.NUMBER (e.g., "1.2.3" or "1.2.3-alpha.4"). No leading zeros allowed, prerelease identifiers must contain only letters, numbers, and hyphens.`,
            );
        } else {
