...
async function checkPackageJson(context) {
    ...
    if (context.packageJson.name !== `iobroker.${adapterName.toLowerCase()}`) {
        context.errors.push(`[E020] Name of adapter in package.json must be lowercase and be equal to "iobroker.${adapterName.toLowerCase()}". Now is "${context.packageJson.name}"`);
    } else {
    ...
    for (const dependency in context.packageJson.dependencies) {
        ...
        !context.packageJson.dependencies[dependency].startsWith('>')
            ...
            if (context.packageJson.dependencies[dependency].toLowerCase().includes('github.com')) {
                ...
                context.warnings.push(`[W047] dependency should not require a specific version. Use "~1.2.3" or "^1.2.3" syntax. Please update "${dependency}:${context.packageJson.dependencies[dependency]}"`);
            }
        }
    }
    ...
    for (const dependency in context.packageJson.devDependencies) {
        ...
        !context.packageJson.devDependencies[dependency].startsWith('>')
            ...
            if (context.packageJson.devDependencies[dependency].toLowerCase().includes('github.com')) {
                ...
                context.warnings.push(`[W048] devDependency should not require a specific version. Use "~1.2.3" or "^1.2.3" syntax. Please update "${dependency}:${context.packageJson.devDependencies[dependency]}"`);
            }
        }
    }
