...
async function checkPackageJson(context) {
    ...
    for (const dependency in context.packageJson.dependencies) {
        if (!context.packageJson.dependencies[dependency].match(/^(\^|~|>|>=|<|<=)?\d+\.\d+\.\d+$/gm)) {
            context.errors.push(
                `[E054] malformed dependency detected "${dependency}":${context.packageJson.dependencies[dependency]}". Please fix at package.json.`,
            );
        } else if (
    ...
    for (const dependency in context.packageJson.devDependencies) {
        if (!context.packageJson.devDependencies[dependency].match(/^(\^|~|>|>=|<|<=)?\d+\.\d+\.\d+$/gm)) {
            context.errors.push(
                `[E055] malformed dependency detected "${dependency}":${context.packageJson.devDependencies[dependency]}". Please fix at package.json.`,
            );
        } else if (
