...
// *** TODO ***
    ...
    for (const dependency in dependenciesPackageJson) {
        ...
        console.log(`    exists: ${exists}, optional: ${optional}, alternatives: [${alternatives}]`);
        ...
        if (exists) {
            ...
            if (found) {
                ...
                if (suggested) {
                    context.warnings.push(
                        `[S061] No dependency declared for ${dependency}. Please consider adding "${dependency}":"${recommendedVersion}" to dependencies at package.json`,
                    );
                }
            } else {
                context.errors.push(
    ...
    for (const dependency in devDependenciesPackageJson) {
        console.log(`checking devDependency ${dependency} ...`);
        ...
        if (exists) {
            ...
            if (found) {
                ...
                if (suggested) {
                    context.warnings.push(
                        `[S062] No devDependency declared for ${dependency}. Please consider adding "${dependency}":"${recommendedVersion}" to devDependencies at package.json`,
                    );
                }
            } else {
                context.errors.push(
