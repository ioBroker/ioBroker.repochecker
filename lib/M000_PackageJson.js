...
async function checkPackageJson(context) {
    ...
    let adapterName = '';
    if (!m || !m[1]) {
        context.errors.push(`[E0004] No adapter name found in URL: ${context.githubUrlOriginal}`);
    } else {
    ...
    if (!context.packageJson.repository) {
        ...
        if (context.packageJson.repository && typeof context.packageJson.repository === 'object') {
            ...
            if (!allowedRepoUrls.includes(context.packageJson.repository.url)) {
                context.errors.push(
                    `[E0019] Invalid repository URL in package.json: ${context.packageJson.repository.url}. Expected: ${context.githubApiData.ssh_url} or ${context.githubApiData.clone_url}`,
                );
            } else {
            ...
            if (!allowedRepoUrls.includes(context.packageJson.repository)) {
                context.errors.push(
                    `[E0019] Invalid repository URL in package.json: ${context.packageJson.repository}. Expected: ${context.githubApiData.ssh_url} or ${context.githubApiData.clone_url}`,
                );
            } else {
                context.checks.push('Repository URL is valid in package.json');
            }
    ...
    for (const dependency in dependenciesPackageJson) {
        ...
        if (exists) {
            ...
            if (!compareVersions.compare(dependencyVersion, requiredVersion, '>=')) {
                context.errors.push(
                    `[E0033] ${dependency} ${dependencyVersion} specified. ${requiredVersion} is required as minimum, ${recommendedVersion} is recommended. Please update dependencies at package.json`,
                );
            } else if (!compareVersions.compare(dependencyVersion, recommendedVersion, '>=')) {
                context.warnings.push(
                    `[W0034] ${dependency} ${dependencyVersion} specified. ${recommendedVersion} is recommended. Please consider updating dependencies at package.json`,
                );
            } else {
    ...
    for (const dependency in devDependenciesPackageJson) {
        ...
        if (exists) {
            ...
            if (!compareVersions.compare(dependencyVersion, requiredVersion, '>=')) {
                context.errors.push(
                    `[E0036] ${dependency} ${dependencyVersion} specified. ${requiredVersion} is required as minimum,  ${recommendedVersion} is recommended. Please update devDependencies at package.json`,
                );
            } else if (!compareVersions.compare(dependencyVersion, recommendedVersion, '>=')) {
                context.warnings.push(
                    `[W0037] ${dependency} ${dependencyVersion} specified. ${recommendedVersion} is recommended. Please consider updating devDependencies at package.json`,
                );
            } else {
            ...
            if (found) {
                ...
                context.errors.push(
                    `[E0035] No devDependency declared for ${dependency}. Please add "${dependency}":"${recommendedVersion}" to devDependencies at package.json`,
                );
            }
        }
    }
...
// [E0008] No "name" key found in package.json.
...
// [061] Invalid version format "${context.packageJson.version}". Version must follow semver format: MAJOR.MINOR.PATCH or MAJOR.MINOR.PATCH-PRERELEASE.NUMBER (e.g., "1.2.3" or "1.2.3-alpha.4"). No leading zeros allowed, prerelease identifiers must contain only letters, numbers, and hyphens.
